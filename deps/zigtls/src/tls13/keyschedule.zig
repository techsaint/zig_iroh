...
pub fn deriveLabelInto(
    ...
    var info: [2 + 1 + tls13_prefix.len + 255 + 1 + 255]u8 = undefined;
    std.debug.assert(label.len + tls13_prefix.len <= 255);
    ...
    @memcpy(info[3 .. 3 + tls13_prefix.len], tls13_prefix);
    var cursor: usize = 3 + tls13_prefix.len;
    @memcpy(info[cursor .. cursor + label.len], label);
    cursor += label.len;
    info[cursor] = @intCast(context.len);
    cursor += 1;
    @memcpy(info[cursor .. cursor + context.len], context);
    cursor += context.len;
    switch (suite) {
        .tls_aes_128_gcm_sha256, .tls_chacha20_poly1305_sha256 => HkdfSha256.expand(out, info[0..cursor], secret),
        .tls_aes_256_gcm_sha384 => HkdfSha384.expand(out, info[0..cursor], secret),
    }
}
