...
pub const Tags = struct {
    ...
    pub fn rename(self: *Tags, from: []const u8, to: []const u8) Error!void {
        ...
        // `set` allocates before mutation. Only remove the source after the
        // destination is safely installed, so an OOM cannot lose the tag.
...
test "tags smoke mem" {
    ...
    try tags.rename("f", "f");
    try std.testing.expect(tags.get("f") != null);
}
