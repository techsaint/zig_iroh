...
/// Application stream error codes (iroh-blobs `protocol.rs`).
/// Used when a provider policy aborts a request via stream reset.
pub const ERR_PERMISSION: u64 = 1;
pub const ERR_LIMIT: u64 = 2;
pub const ERR_INTERNAL: u64 = 3;
...
fn decodeRangeSpec(
    ...
    const len = try postcard.readSliceHeader(r);
    // Cap by heap footprint (each width is a u64). The old `len > MAX_MESSAGE_SIZE`
    // check allowed allocating up to MAX_MESSAGE_SIZE * 8 ≈ 8 MiB before the
    // streaming Get/GetMany body failed mid-read (F23).
    if (len > MAX_MESSAGE_SIZE / @sizeOf(u64)) return error.MessageTooLarge;
    // Each postcard u64 needs at least one byte. Fixed Observe/Push bodies are
    // fully buffered, so reject impossible counts before allocating.
    if (fixed_frame and len > r.bufferedLen()) return error.EndOfStream;
    // Streaming Get/GetMany: same min-byte budget when the body is already
    // buffered; when not, the /8 cap above still bounds the heap alloc.
    if (!fixed_frame and r.bufferedLen() > 0 and len > r.bufferedLen()) return error.EndOfStream;
...
fn decodeChunkRangesSeq(
    ...
    const len = try postcard.readSliceHeader(r);
    // Bound entry-table heap size AND require ≥2 wire bytes/entry (delta +
    // RangeSpec length). Streaming Get/GetMany previously only had the /2
    // count check, which still allowed multi-MB Entry tables (F23).
    const max_entries = @min(MAX_MESSAGE_SIZE / 2, MAX_MESSAGE_SIZE / @sizeOf(range_spec.ChunkRangesSeq.Entry));
    if (len > max_entries) return error.MessageTooLarge;
    ...
    if (!fixed_frame and r.bufferedLen() > 0 and len > r.bufferedLen() / 2) return error.EndOfStream;
...
test "fixed Observe and Push frames reject impossible counts before allocation" {
    ...
    // Cap is now MAX_MESSAGE_SIZE/@sizeOf(u64); MAX_MESSAGE_SIZE widths reject
    // as MessageTooLarge before the fixed-frame EndOfStream path.
    try std.testing.expectError(
        error.MessageTooLarge,
...
test "F23 streaming Get/GetMany reject oversized LEB before multi-MB alloc" {
    const alloc = std.testing.allocator;
    const FailingAllocator = std.testing.FailingAllocator;

    // Get: tag + hash + ChunkRangesSeq(1 entry) + RangeSpec claiming
    // MAX_MESSAGE_SIZE widths with no width bodies. Pre-fix allocated
    // MAX_MESSAGE_SIZE * 8 ≈ 8 MiB then hit EndOfStream; post-fix rejects
    // with MessageTooLarge and near-zero heap use.
    {
        var buf: [64]u8 = undefined;
        var w: std.Io.Writer = .fixed(&buf);
        try w.writeByte(@intFromEnum(RequestType.get));
        try w.splatByteAll(0xda, 32);
        try postcard.writeSliceHeader(&w, 1); // one ChunkRangesSeq entry
        try postcard.writeU64(&w, 0); // offset delta
        try postcard.writeSliceHeader(&w, MAX_MESSAGE_SIZE); // malicious width count
        var reader: std.Io.Reader = .fixed(w.buffered());
        var counter = FailingAllocator.init(alloc, .{});
        try std.testing.expectError(
            error.MessageTooLarge,
            GetRequest.decode(counter.allocator(), &reader),
        );
        try std.testing.expect(counter.allocated_bytes < 64 * 1024);
    }

    // GetMany: claim a huge ChunkRangesSeq entry count (old /2 cap still
    // allowed multi-MB Entry tables).
    {
        var buf: [48]u8 = undefined;
        var w: std.Io.Writer = .fixed(&buf);
        try w.writeByte(@intFromEnum(RequestType.get_many));
        try postcard.writeSliceHeader(&w, 0); // zero hashes
        try postcard.writeSliceHeader(&w, MAX_MESSAGE_SIZE / 2); // malicious entry count
        var reader: std.Io.Reader = .fixed(w.buffered());
        var counter = FailingAllocator.init(alloc, .{});
        try std.testing.expectError(
            error.MessageTooLarge,
            GetManyRequest.decode(counter.allocator(), &reader),
        );
        try std.testing.expect(counter.allocated_bytes < 64 * 1024);
    }
}
