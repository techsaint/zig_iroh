...
pub fn encodeTicketString(allocator: std.mem.Allocator, endpoint: addr.EndpointAddr) Error![]u8 {
    const max_ticket = encodedSizeBound(endpoint);
    const ticket_buf = allocator.alloc(u8, max_ticket) catch return error.OutOfMemory;
    defer allocator.free(ticket_buf);
    const ticket_bytes = try encodeTicketBytes(endpoint, ticket_buf);

    const b32_max = (ticket_bytes.len * 8 + 4) / 5;
    const b32_buf = allocator.alloc(u8, b32_max) catch return error.OutOfMemory;
    defer allocator.free(b32_buf);
...
pub fn decodeTicketString(allocator: std.mem.Allocator, s: []const u8) Error!addr.EndpointAddr {
    ...
    const upper_buf = allocator.alloc(u8, rest.len) catch return error.OutOfMemory;
    defer allocator.free(upper_buf);
    for (rest, 0..) |c, i| upper_buf[i] = std.ascii.toUpper(c);
    const raw_max = (rest.len * 5 + 7) / 8;
    const raw = allocator.alloc(u8, raw_max) catch return error.OutOfMemory;
    defer allocator.free(raw);
    const n = try base32NoPadDecode(upper_buf, raw);
    return decodeTicketBytes(allocator, raw[0..n]);
}

fn encodedSizeBound(endpoint: addr.EndpointAddr) usize {
    var size: usize = 1 + 32 + 10;
    for (endpoint.addrs) |ta| {
        switch (ta) {
            .relay => |relay| size += 10 + relay.asString().len,
            .ip => size += 1 + 1 + 16 + 5,
            .custom => |custom| size += 1 + 1 + 1 + 30 + 10 + custom.data.len,
        }
    }
    return size;
}
...
pub fn base32NoPadDecode(s: []const u8, out: []u8) Error!usize {
    var acc: u32 = 0;
    var nbits: u32 = 0;
    var oi: usize = 0;
    for (s) |c| {
...
pub fn base32NoPadEncode(input: []const u8, out: []u8) Error!usize {
    var acc: u32 = 0;
    var nbits: u32 = 0;
    var oi: usize = 0;
    for (input) |byte| {
...
test "ticket non-Variant1 tag byte rejects UnsupportedVariant" {
    const allocator = std.testing.allocator;
    const sk = key.SecretKey.fromBytes(.{0x44} ** 32);
    const id = sk.public();
    var relay = try addr.RelayUrl.parse(allocator, "https://relay.example.com.");
    defer relay.deinit(allocator);
    var endpoint = try addr.EndpointAddr.fromParts(allocator, id, &.{.{ .relay = relay }});
    ...
    var buf: [256]u8 = undefined;
    const encoded = try encodeTicketBytes(endpoint, &buf);
    try std.testing.expectEqual(@as(u8, 0), encoded[0]); // Variant1 tag
    // decodeTicketBytes: bytes[0] != 0 -> UnsupportedVariant. The gate fails if an
    // unsupported tag byte is ever accepted or misclassified as another error.
    encoded[0] = 0x01;
    try std.testing.expectError(error.UnsupportedVariant, decodeTicketBytes(allocator, encoded));
    encoded[0] = 0xFF;
    try std.testing.expectError(error.UnsupportedVariant, decodeTicketBytes(allocator, encoded));
}

test "ticket over-long count varint rejects VarintOverflow" {
    const allocator = std.testing.allocator;
    const sk = key.SecretKey.fromBytes(.{0x55} ** 32);
    const id = sk.public();
    var relay = try addr.RelayUrl.parse(allocator, "https://relay.example.com.");
    defer relay.deinit(allocator);
    var endpoint = try addr.EndpointAddr.fromParts(allocator, id, &.{.{ .relay = relay }});
    ...
    var buf: [256]u8 = undefined;
    const encoded = try encodeTicketBytes(endpoint, &buf);
    // Ticket bytes: Variant1 tag[1] ++ id[32] ++ count varint (offset 33); the bare
    // EndpointAddr postcard form drops the tag byte (count varint at offset 32).
    const ticket_count_off = 1 + 32;
    var crafted: [ticket_count_off + 10]u8 = undefined;
    @memcpy(crafted[0..ticket_count_off], encoded[0..ticket_count_off]);
    // Ten continuation bytes: readVarint's fuzz-sourced shift guard (shift >= 63)
    // must fire on the 10th byte — accepting the varint or trapping both fail here.
    @memset(crafted[ticket_count_off..], 0x80);
    try std.testing.expectError(error.VarintOverflow, decodeTicketBytes(allocator, &crafted));
    try std.testing.expectError(error.VarintOverflow, decodeEndpointAddr(allocator, crafted[1..]));
}

test "endpoint addr unknown TransportAddr tag rejects UnsupportedVariant" {
    const allocator = std.testing.allocator;
    const sk = key.SecretKey.fromBytes(.{0x66} ** 32);
    const id = sk.public();
    var relay = try addr.RelayUrl.parse(allocator, "https://relay.example.com.");
    defer relay.deinit(allocator);
    var endpoint = try addr.EndpointAddr.fromParts(allocator, id, &.{.{ .relay = relay }});
    ...
    // One-addr endpoint: id[32] ++ count=1 (one varint byte) ++ the TransportAddr
    // tag varint; decodeTransportAddr's `else` arm must reject unknown tags.
    const addr_tag_off_in_endpoint = 32 + 1;
    var buf: [256]u8 = undefined;
    const encoded = try encodeEndpointAddr(endpoint, &buf);
    try std.testing.expect(encoded.len > addr_tag_off_in_endpoint);
    try std.testing.expectEqual(@as(u8, 0), encoded[addr_tag_off_in_endpoint]); // relay tag
    encoded[addr_tag_off_in_endpoint] = 0x03;
    try std.testing.expectError(error.UnsupportedVariant, decodeEndpointAddr(allocator, encoded));
    encoded[addr_tag_off_in_endpoint] = 0x7F; // unknown single-byte varint tag 127
    try std.testing.expectError(error.UnsupportedVariant, decodeEndpointAddr(allocator, encoded));

    // Same rejection through the ticket envelope (tag byte shifts by one).
    var tbuf: [256]u8 = undefined;
    const ticket_bytes = try encodeTicketBytes(endpoint, &tbuf);
    try std.testing.expectEqual(@as(u8, 0), ticket_bytes[addr_tag_off_in_endpoint + 1]);
    ticket_bytes[addr_tag_off_in_endpoint + 1] = 0x03;
    try std.testing.expectError(error.UnsupportedVariant, decodeTicketBytes(allocator, ticket_bytes));
}
