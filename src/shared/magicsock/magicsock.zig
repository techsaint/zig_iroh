...
pub fn selectPath(candidates: []const Candidate, current_index: ?usize) ?usize {
    ...
    const current_score: ?u64 = if (current_index) |current|
        if (candidates[current].state == .succeeded)
            normalizedScore(candidates[current])
        else
            null
    else
        null;
    for (candidates, 0..) |candidate, i| {
        ...
        if (current_score) |cs| {
            if (i != current_index.?) {
                if (score + switch_hysteresis_us >= cs) continue;
            }
        }
        if (score < best_score) {
            best_score = score;
...
fn normalizedScore(candidate: Candidate) u64 {
    var score: u64 = candidate.rtt_us orelse std.math.maxInt(u32);
...
test "selectPath normalizes current score for hysteresis (relay vs direct)" {
    const addr4: net.IpAddress = .{ .ip4 = .{ .bytes = .{ 192, 0, 2, 1 }, .port = 4433 } };
    const candidates = [_]Candidate{
        .{ .seq = 1, .address = addr4, .state = .succeeded, .rtt_us = 50_000, .kind = .relay },
        .{ .seq = 2, .address = addr4, .state = .succeeded, .rtt_us = 100_000, .kind = .direct_ipv4 },
    };
    // Current is relay (index 0): normalized score = 50_000 + 1e9 = 1_000_050_000.
    // Challenger direct (index 1): normalized score = 100_000.
    // 100_000 + 5_000 hysteresis < 1_000_050_000 → challenger wins.
    const selected = selectPath(&candidates, 0);
