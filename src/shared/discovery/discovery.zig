...
fn wallClockMicros() u64 {
    var ts: std.posix.timespec = undefined;
    const rc = std.posix.system.clock_gettime(std.posix.CLOCK.REALTIME, &ts);
    if (std.posix.errno(rc) != .SUCCESS) return 0;
    return @as(u64, @intCast(ts.sec)) * std.time.us_per_s + @as(u64, @intCast(ts.nsec)) / std.time.ns_per_us;
}

var last_timestamp: std.atomic.Value(u64) = .init(0);

pub const Timestamp = struct {
    micros: u64,

    /// Wall-clock microseconds since UNIX epoch, strictly monotonic.
    ///
    /// Matches real iroh's `Timestamp::now()` (pkarr.rs): wall-clock micros
    /// with a CAS loop guaranteeing each call returns at least `last + 1`,
    /// so DHT/pkarr recency comparisons against real iroh nodes are correct.
    /// Prefer an explicit `Timestamp{ .micros = N }` in tests for determinism.
    pub fn now() Timestamp {
        const wall = wallClockMicros();
        var last = last_timestamp.load(.monotonic);
        while (true) {
            const next = @max(wall, last + 1);
            if (last_timestamp.cmpxchgWeak(last, next, .monotonic, .monotonic)) |actual| {
                last = actual;
            } else {
                return .{ .micros = next };
            }
...
pub fn publishPkarrRelayWithOptions(
    ...
    // Explicit request flow instead of `client.fetch`: RFC 9110 6.4.1 says
    // 1xx/204/304 responses carry no body regardless of framing headers, but
    // Zig 0.16 `Response.reader()` only special-cases the request METHOD
    // (`responseHasBody`); for a PUT answered `204 No Content` on a
    // keep-alive connection with no Content-Length, `fetch`'s body drain
    // builds a read-until-EOF reader and hangs forever (observed against the
    // real https://dns.iroh.link relay and httpbin /status/204). Read the
    // body only for statuses that carry one.
    ...
    const payload = packet.relayPayload();
    ...
    const bodiless = status.class() == .informational or
...
test "Timestamp.now returns wall-clock micros and is strictly monotonic" {
    const a = Timestamp.now();
    const b = Timestamp.now();
    try std.testing.expect(b.micros > a.micros);
    // 2020-01-01T00:00:00Z in micros — any real wall-clock reading is above this.
    const epoch_2020_us: u64 = 1_577_836_800 * std.time.us_per_s;
    try std.testing.expect(a.micros > epoch_2020_us);
}
...
test "publishPkarrRelay returns on bare 204 keep-alive response (std.http 204 drain hang regression)" {
    // RFC 9110 6.4.1: a 204 response has no body regardless of framing
    // headers. The real pkarr relay (axum/hyper) answers a successful PUT
    // with a bare "HTTP/1.1 204 No Content\r\n\r\n" and keeps the
    // connection alive. Zig 0.16 std.http.Client.fetch then drains a body
    // that never comes (read-until-EOF on a keep-alive socket) and hangs
    // forever. The explicit request flow in publishPkarrRelayWithOptions
    // must return on the head alone. The fixture HOLDS the connection for
    // 3s before closing so the unfixed path also terminates (bounded), just
    // slowly: mutation-RED against the hang is the elapsed assertion.
    const allocator = std.testing.allocator;
    ...
    const Fixture = struct {
        ...
        fn run(self: *@This()) void {
            ...
            // Consume the request head only; the tiny PUT body stays in the
            // kernel buffer (fits easily) and is discarded on close.
            while (true) {
            ...
            // Bare 204: no Content-Length, no Connection header — the exact
            // n0 relay response shape that hung std.http.Client.fetch.
            writer.interface.writeAll("HTTP/1.1 204 No Content\r\n\r\n") catch return;
            writer.interface.flush() catch return;
            // Hold the connection open (keep-alive) so a body drain blocks;
            // close after 3s to bound the unfixed path.
            self.io.sleep(std.Io.Duration.fromMilliseconds(3000), .awake) catch {};
        }
    };
    var fixture: Fixture = .{ .listener = &listener, .io = io };
    const fixture_thread = try std.Thread.spawn(.{}, Fixture.run, .{&fixture});
    defer fixture_thread.join();

    const relay_url = try std.fmt.allocPrint(allocator, "http://127.0.0.1:{d}", .{port});
    ...
    const Publisher = struct {
        ...
        fn run(self: *@This()) void {
            ...
            defer client.deinit();
            const secret = root.SecretKey.fromBytes(.{0xE7} ** 32);
            const direct = std.Io.net.IpAddress.parse("127.0.0.1", 4242) catch unreachable;
            const info = EndpointInfo.fromParts(self.allocator, secret.public(), &.{.{ .ip = direct }}, null) catch unreachable;
            defer info.deinit(self.allocator);
            publishPkarrRelay(self.allocator, &client, self.url, secret, info, DEFAULT_TTL, .{ .micros = 1 }) catch |err| {
                self.err = err;
            };
        }
    };
    var publisher: Publisher = .{ .allocator = allocator, .io = io, .url = relay_url };
    const started = std.Io.Clock.Timestamp.now(io, .awake);
    const publish_thread = try std.Thread.spawn(.{}, Publisher.run, .{&publisher});
    publish_thread.join();
    const elapsed_ns = started.durationTo(std.Io.Clock.Timestamp.now(io, .awake)).raw.toNanoseconds();

    try std.testing.expect(publisher.err == null);
    // Must return on the response head; the unfixed drain unblocks only when
    // the fixture closes at 3s.
