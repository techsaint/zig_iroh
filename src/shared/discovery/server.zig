...
pub const ServerOptions = struct {
    ...
    /// Per-connection read timeout in milliseconds. A watchdog thread shuts
    /// down the stream after this deadline so one idle client cannot block
    /// the serial accept loop indefinitely. 0 disables the watchdog.
    read_timeout_ms: u64 = 10_000,
};
...
pub fn serve(io: std.Io, allocator: std.mem.Allocator, options: ServerOptions) !void {
    ...
    while (true) {
        ...
        handleAcceptedStream(io, allocator, &store, stream, &limiter, options.read_timeout_ms);
    }
}
...
fn handleAcceptedStream(
    ...
    const wd_thread: ?std.Thread = if (read_timeout_ms > 0)
        std.Thread.spawn(.{}, ReadWatchdog.run, .{&watchdog}) catch null
    else
        null;
    defer {
...
fn testServeN(ctx: *TestServerContext) !void {
    ...
    while (i < ctx.count) : (i += 1) {
        ...
        handleAcceptedStream(ctx.io, ctx.allocator, ctx.store, stream, &limiter, 0);
    }
}
