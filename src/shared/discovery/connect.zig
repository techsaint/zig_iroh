...
const tr = @import("transport"); // fork-isolation S2: via the named door
...
) ConnectByIdError!@TypeOf(transport.connect(@as(tr.NodeAddr, undefined)) catch unreachable) {
...
/// Records what `connectById` dialled. `connectById` takes the transport as
/// `anytype`, so this only needs the method surface the resolver path calls —
/// no vtable, no type erasure.
const FakeTransport = struct {
    ...
    const Connection = struct {
        ...
        fn openBi(_: Connection) tr.Error!void {
            return error.NotConnected;
        }
        fn acceptBi(_: Connection) tr.Error!void {
            return error.NotConnected;
        }
        fn openUni(_: Connection) tr.Error!void {
            return error.NotConnected;
        }
        fn acceptUni(_: Connection) tr.Error!void {
            return error.NotConnected;
        }
        fn remoteNodeId(self: Connection) key.NodeId {
            return self.owner.expected_remote;
        }
        fn alpn(_: Connection) ?[]const u8 {
            return "fake";
        }
        fn remoteAddress(_: Connection) ?std.Io.net.IpAddress {
            return null; // FakeTransport has no socket path
        }
        fn close(_: Connection) void {}
        fn io(_: Connection) std.Io {
            return std.testing.io;
        }
    };

    fn transport(self: *FakeTransport) *FakeTransport {
        return self;
    }

    fn connect(self: *FakeTransport, peer: tr.NodeAddr) tr.Error!Connection {
        self.calls += 1;
    ...
    fn accept(_: *FakeTransport) tr.Error!Connection {
        return error.NotConnected;
    }

    fn localNodeId(self: *FakeTransport) key.NodeId {
        return self.expected_remote;
    }

    fn io(_: *FakeTransport) std.Io {
        return std.testing.io;
    }
};
