...
pub const ServerConfig = struct {
    ...
    /// SNI/ALPN-driven per-connection certificate selection (ACME mode). When
    /// set, the static PEM pair must be null — the selector decides what each
    /// handshake serves, including the TLS-ALPN-01 challenge certs, and a
    /// connection that negotiates `acme-tls/1` is closed right after the
    /// handshake (never enters the WebSocket/auth path).
    tls_cert_selector: ?tls_wrapper.CertSelector = null,
...
fn encodeRelayFrame(allocator: std.mem.Allocator, msg: proto.RelayToClientMsg) !OutboundFrame {
    // Regression lane-03 H2: encode through a fixed stack buffer (same pattern
    // as relay client send()) and dupe exactly once — the previous
    // `std.Io.Writer.Allocating` + `toOwnedSlice` grew an intermediate heap
    // buffer per outbound frame on the relay hot path. A message that does not
    // fit MAX_FRAME_SIZE could never go on the wire anyway, so NoSpaceLeft is
    // a correct encode error here.
    var frame_buf: [proto.MAX_FRAME_SIZE]u8 = undefined;
    var frame_writer = std.Io.Writer.fixed(&frame_buf);
    try proto.encodeRelayToClient(msg, &frame_writer);
    return .{ .op = .binary, .payload = try allocator.dupe(u8, frame_writer.buffered()) };
}
...
pub const Server = struct {
    ...
    fn handleClient(self: *Server, stream: std.Io.net.Stream) !void {
        ...
        if (self.config.tls_cert_selector) |selector| {
            tls_srv = tls_wrapper.TlsServer.acceptWithSelector(self.allocator, self.io, stream, selector) catch |err| {
                std.debug.print("[server] TLS handshake failed: {}\n", .{err});
                return;
            };
            // An ACME TLS-ALPN-01 validation probe got its challenge cert at
            // the handshake; it is never a relay client (RFC 8737). Close it
            // without entering the WebSocket/auth path.
            if (tls_srv.?.alpnProtocol()) |alpn| {
                if (std.mem.eql(u8, alpn, tls_wrapper.acme_tls_alpn)) return;
            }
            reader = tls_srv.?.reader();
            writer = tls_srv.?.writer();
        } else if (self.config.tls_cert_path != null and self.config.tls_key_path != null) {
