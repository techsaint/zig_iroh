...
//!
//! Reference: `iroh/iroh-relay/src/protos/common.rs` (FrameType enum)
//!            `iroh/iroh-relay/src/protos/relay.rs` (data frames)
...
/// Reference: `iroh/iroh-relay/src/protos/common.rs:18-65`
...
/// Reference: `iroh/iroh-relay/src/protos/relay.rs:252-292`
...
// Frame-by-frame audit coverage (relay-wire-frame-by-frame-audit-coverage):
// every tag 0..13 exists as a FrameType discriminant, has the correct
// version gate, and is reachable from encode/decode paths exercised by the
// snapshot + round-trip tests above. Mutation-red: remove a tag from the
// enum or change its integer value → this test fails; drop a version gate
// (health-in-v2 / status-in-v1) → validForVersion asserts fail.
test "frame-by-frame audit: tags 0..13 dense and version-gated" {
    const expected = [_]struct { FrameType, u6 }{
        .{ .server_challenge, 0 },
        .{ .client_auth, 1 },
        .{ .server_confirms_auth, 2 },
        .{ .server_denies_auth, 3 },
    ...
    try testing.expectEqual(@as(usize, 14), expected.len);
    for (expected) |pair| {
        try testing.expectEqual(pair[1], @intFromEnum(pair[0]));
        // Dense: every integer 0..13 is a known tag (no holes).
        const round: FrameType = @enumFromInt(pair[1]);
        try testing.expectEqual(pair[0], round);
    }
    // Version gates: Health is V1-only; Status is V2-only; the rest both.
    try testing.expect(FrameType.health.validForVersion(.v1));
    try testing.expect(!FrameType.health.validForVersion(.v2));
    try testing.expect(FrameType.status.validForVersion(.v2));
    try testing.expect(!FrameType.status.validForVersion(.v1));
    try testing.expect(FrameType.ping.validForVersion(.v1));
    try testing.expect(FrameType.ping.validForVersion(.v2));
    try testing.expect(FrameType.endpoint_gone.validForVersion(.v2));
    try testing.expect(FrameType.restarting.validForVersion(.v1));
    // Unknown tag >13 must be rejected by the inbound decoder.
    const unknown = [_]u8{14}; // single-byte varint tag 14, empty payload
    try testing.expectError(error.UnknownFrameType, decodeRelayToClient(&unknown, .v2));
}
