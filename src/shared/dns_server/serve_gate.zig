...
pub fn main(init: std.process.Init) !void {
    ...
    try zig_iroh.dns_server.server.serveDetachedHandlerLifetimeGateOnce(allocator, io);
    try zig_iroh.dns_server.server.serveMainlineResolverLifetimeGateOnce(allocator, io);
    ...
    try stdout_writer.interface.print(
        "dns-server-serve-gate: PASS (UDP A + TCP DNS + DoH; AXFR refused, " ++
            ...
            "slow-client deadline, bounded detached-handler lifetime, mainline resolver lifetime, " ++
            "docker port coherence, HTTPS self_signed DoH, " ++
