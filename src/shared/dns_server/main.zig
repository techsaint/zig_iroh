...
pub fn main(init: std.process.Init) !void {
    ...
    // The mainline resolver (when mainline_enabled) is created and owned by
    // the server's Shared state — torn down only after the last detached
    // handler releases, so there is no resolver defer here to order wrongly.
