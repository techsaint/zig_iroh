...
//!
//! Mirrors `iroh-blobs/src/hash.rs` (n0-computer/iroh, Apache-2.0/MIT).
...
test "hash digests match reference-generated fixture class" {
    // Fixture provenance: tools/iroh_base_probe `hash` class -> vectors/iroh-base/
    // fixtures.json -> (json_to_zig.py) -> iroh_base_fixtures.zig#hash. Digests were
    // generated by the pinned Rust blake3 crate (=1.8.5) — the exact dependency
    // iroh-blobs `Hash` wraps (original/Cargo.lock) — over the official BLAKE3
    // test-vector input pattern input[i] = i % 251. Mutation-RED: any digest drift
    // (wrong init/chunk/finalize) flips a case.
    var input: [3073]u8 = undefined;
    for (&input, 0..) |*b, i| b.* = @intCast(i % 251);
    for (fixtures.hash) |case| {
        const h = Hash.of(input[0..case.len]);
        try std.testing.expectEqualStrings(case.digest_hex, &h.toHex());
        // The reference text forms parse back to the same digest.
        try std.testing.expect((try Hash.parse(case.digest_hex)).eql(h));
        try std.testing.expect((try Hash.parse(case.base32_nopad)).eql(h));
        // The empty-input fixture is the iroh-blobs-pinned Hash::EMPTY constant —
        // the zero-degree tie between this class and the reference checkout.
        if (case.len == 0) try std.testing.expectEqualStrings(&Hash.empty.toHex(), case.digest_hex);
    }
}
