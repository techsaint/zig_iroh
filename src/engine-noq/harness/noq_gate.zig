...
/// The returned slice points INTO `batch`'s heap-backed storage — the caller
/// owns the batch and must keep it alive while the slice is in use.
fn receiveOne(ep: *noq_ep.Endpoint, batch: *noq_ep.RawReceiveScratch, rounds: usize) !?[]const u8 {
    var i: usize = 0;
    while (i < rounds) : (i += 1) {
        var payloads: [1][]const u8 = undefined;
        var codepoints: [1]?@import("shared").udp_cmsg.EcnCodepoint = undefined;
...
/// The returned slice points INTO `batch`'s heap-backed storage — the caller
/// owns the batch and must keep it alive while the slice is in use.
fn receiveResetForTest(ep: *noq_ep.Endpoint, batch: *noq_ep.RawReceiveScratch, tail: [packet.stateless_reset_token_len]u8, rounds: usize) !?[]const u8 {
    var i: usize = 0;
    while (i < rounds) : (i += 1) {
        var payloads: [4][]const u8 = undefined;
        var codepoints: [4]?@import("shared").udp_cmsg.EcnCodepoint = undefined;
        const n = ep.receiveRawForTest(&payloads, &codepoints, 20 * std.time.ns_per_ms, batch) catch |err| switch (err) {
...
test "E3: stateless reset SEND — HMAC-consistent, rate-limited, pad-below-inciting, unknown-CID-only — real sockets" {
    ...
    // Heap-backed receive scratch for the reset legs; the returned payloads
    // point into it, so it outlives every use below (test-scoped).
    const reset_batch = try noq_ep.RawReceiveScratch.create(allocator, client_noq.gro_segments);
    defer reset_batch.destroy(allocator);
    ...
    const reset = (try receiveResetForTest(client_noq, reset_batch, expected_token, 12)) orelse return error.UnexpectedState;
    ...
    const reset2 = (try receiveResetForTest(client_noq, reset_batch, expected_token2, 12)) orelse return error.UnexpectedState;
...
test "5d-B: server learns a verified peer, mints a fresh CID, and rejects a spoofed RPK" {
    // Product-aware (picotls AND zigtls): bare create defaults tls_backend=.picotls
    // and under noq-zigtls dies at connection.zig:1252. Do NOT picotls-skip —
    // interop-noq counts this leg for both noq products.
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    ...
    defer server_any.deinit();
    const honest_client = try factory.createForProduct(allocator, io, client_key, alpn, .{});
    defer honest_client.deinit();
    try std.testing.expectEqual(factory.productTlsBackend(), server_any.tlsBackend());
    try std.testing.expectEqual(factory.productTlsBackend(), honest_client.tlsBackend());
    const server_noq = client_any_noq(server_any);
    ...
    const spoof_client = try factory.createForProduct(allocator, io, signing_key, alpn, .{
        .certificate_public_key_override = spoofed_key.public(),
    });
    defer spoof_client.deinit();
    try std.testing.expectEqual(factory.productTlsBackend(), spoof_client.tlsBackend());
...
test "passive migration — probe to the new address (1200-padded), verbatim echo switches the path — real sockets" {
    ...
    const batch = try noq_ep.RawReceiveScratch.create(allocator, spy_noq.gro_segments);
