...
pub const Endpoint = struct {
    ...
    fn removeConnectionReferences(self: *Endpoint, handle: ConnectionHandle) void {
        // Regression lane-02 H2: single pass per map — collect matching keys,
        // then remove. The previous rescan-per-removal loop made teardown
        // O(entries × matching refs) per connection; this is O(entries).
        removeRefs(ConnectionId, &self.connection_ids, handle);
        removeRefs(ConnectionId, &self.connection_ids_initial, handle);
        removeRefs(FourTuple, &self.incoming_connection_remotes, handle);
        removeRefs(SocketAddress, &self.outgoing_connection_remotes, handle);
        removeRefs(ResetKey, &self.connection_reset_tokens, handle);
    }
};

/// Collect every key whose value references `handle`, then remove them after
/// the scan (values are either a bare ConnectionHandle or a ConnHandlePath).
fn removeRefs(comptime K: type, map: anytype, handle: ConnectionHandle) void {
    var to_remove: std.ArrayList(K) = .empty;
    defer to_remove.deinit(map.allocator);
    var it = map.iterator();
    while (it.next()) |entry| {
        const entry_handle: ConnectionHandle = switch (@TypeOf(entry.value_ptr.*)) {
            ConnectionHandle => entry.value_ptr.*,
            ConnHandlePath => entry.value_ptr.handle,
            else => @compileError("removeRefs: unsupported map value type"),
        };
        if (entry_handle.eql(handle)) {
            to_remove.append(map.allocator, entry.key_ptr.*) catch {
                // Teardown cannot fail: on OOM remove this key in place and
                // restart the scan (the removal invalidates the iterator).
