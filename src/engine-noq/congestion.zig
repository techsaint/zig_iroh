//! Congestion controller vtable + factories.
//!
...
/// Fork-isolation S4: the selection enum's DECL moved to
/// `shared/transport_contract.zig` (`CongestionKind`) — the engine-blind
/// factory `Options` names it on every product, so it must live on the shared
/// side of the seam. Alias, not a copy: one decl, one type identity.
pub const Kind = @import("shared").transport_contract.CongestionKind;
