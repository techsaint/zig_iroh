...
//!
//! `Server` is self-referential (`dns` holds pointers into sibling fields).
//! Always construct via out-pointer `init` — never return by value (see
//! the standard Zig self-referential-struct idiom).
...
pub const Server = struct {
    ...
    /// Out-pointer init: pointers inside `dns` target `self`'s stable storage.
    pub fn init(self: *Server, allocator: std.mem.Allocator, io: std.Io, config: config_mod.Config) !void {
        ...
        self.config_live = .init(&self.config);
        self.store.metrics = &self.metrics;
        self.store.max_age_secs = .init(config.packet_max_age_secs);
        self.dns = .{
            .allocator = allocator,
            .config = &self.config,
            .store = &self.store,
            .metrics = &self.metrics,
            .config_source = &self.config_live,
    ...
    /// Attach a background mainline resolver. The answer path only enqueues;
    /// resolved packets land in `ZoneStore` off the fast path.
    pub fn setMainlineResolver(self: *Server, resolver: *mainline_mod.BackgroundResolver) void {
        self.mainline_resolver = resolver;
        self.dns.mainline_resolver = resolver;
    }
    ...
    pub fn deinit(self: *Server) void {
        self.store.deinit();
        // Safe here and only here: `run` has joined every serve thread, so no
        // reader can still hold a retired generation.
    ...
    pub fn run(self: *Server) !void {
        ...
        // Server storage they read.
        var waits: usize = 0;
        while (self.active_handlers.load(.acquire) > 0 and waits < 500) : (waits += 1) {
...
fn httpAcceptLoop(self: *Server, listener: *net.Server) void {
    while (self.running.load(.acquire)) {
        ...
        const thread = std.Thread.spawn(.{}, httpConnThread, .{ self, stream }) catch {
            releaseHandlerSlot(self);
            stream.close(self.io);
            continue;
        };
        thread.detach();
...
fn httpConnThread(self: *Server, stream: net.Stream) void {
    defer releaseHandlerSlot(self);
    defer stream.close(self.io);
    handleHttpStream(self, stream) catch {};
}
...
fn httpsAcceptLoop(self: *Server, listener: *net.Server) void {
    while (self.running.load(.acquire)) {
        ...
        const thread = std.Thread.spawn(.{}, httpsConnThread, .{ self, stream }) catch {
            releaseHandlerSlot(self);
            stream.close(self.io);
            continue;
        };
...
fn httpsConnThread(self: *Server, stream: net.Stream) void {
    defer releaseHandlerSlot(self);
    // TlsServer.close owns the TCP stream close.
    handleHttpsStream(self, stream) catch {
        stream.close(self.io);
    };
}
...
fn handleHttpsStream(self: *Server, stream: net.Stream) !void {
    const cert = self.tls_cert_path orelse return error.SelfSignedListenerNotWired;
    const key = self.tls_key_path orelse return error.SelfSignedListenerNotWired;
    ...
    const wd_thread = armConnWatchdog(
        &watchdog_stream,
        self.io,
        self.config.connection_read_timeout_ms,
        &timed_out,
        &cancel,
        &watchdog,
    );
    ...
    const tls_srv = tls_wrapper.TlsServer.accept(self.allocator, self.io, stream, cert, key) catch {
...
fn handleHttpStream(self: *Server, stream: net.Stream) !void {
    var timed_out = std.atomic.Value(bool).init(false);
    var cancel = std.atomic.Value(bool).init(false);
    var watchdog_stream = stream;
    var watchdog: ConnWatchdog = undefined;
...
fn dnsTcpAcceptLoop(self: *Server, listener: *net.Server) void {
    while (self.running.load(.acquire)) {
        ...
        const thread = std.Thread.spawn(.{}, dnsTcpConnThread, .{ self, stream }) catch {
            releaseHandlerSlot(self);
            stream.close(self.io);
            continue;
        };
...
fn dnsTcpConnThread(self: *Server, stream: net.Stream) void {
    defer releaseHandlerSlot(self);
    defer stream.close(self.io);
    handleDnsTcp(self, stream) catch {
        _ = self.metrics.dns_query_errors.fetchAdd(1, .monotonic);
    };
}

fn handleDnsTcp(self: *Server, stream: net.Stream) !void {
    _ = self.metrics.dns_requests_tcp.fetchAdd(1, .monotonic);
    ...
    const query = try self.allocator.alloc(u8, msg_len);
    ...
    const resp = try self.dns.answer(query);
    defer self.allocator.free(resp);
...
pub fn serveMainlineBackgroundGateOnce(allocator: std.mem.Allocator, io: std.Io) !void {
    ...
    var resolver = mainline_mod.BackgroundResolver.init(allocator, io, &server.store, &server.metrics);
    defer resolver.deinit();
    resolver.setInjectedLookup(@ptrCast(&ctx), Ctx.lookup);
    server.setMainlineResolver(&resolver);
...
pub fn serveCliRoundTripOnce(allocator: std.mem.Allocator, io: std.Io, cli_path: []const u8) !void {
    ...
    const secret_hex = std.fmt.bytesToHex(secret.seed, .lower);
    const z32 = secret.public().toZ32();
