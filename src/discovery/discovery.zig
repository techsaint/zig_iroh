...
pub const EndpointInfo = struct {
    ...
    pub fn fromNodeAddr(
        ...
        return .{
            ...
            .addrs = try cloneTransportAddrs(allocator, node_addr.addrs),
            .user_data = if (user_data) |u| try allocator.dupe(u8, u) else null,
            .owned = true,
        };
    }
    ...
    pub fn fromTxtLookup(
        ...
        return fromTxtStrings(allocator, node_id, txt_values);
    }
    ...
    pub fn fromSignedPacket(allocator: std.mem.Allocator, packet: SignedPacket) !EndpointInfo {
        ...
        return fromTxtStrings(allocator, packet.public_key, values);
    }
...
pub const Timestamp = struct {
    ...
    pub fn now() Timestamp {
        const raw = std.time.microTimestamp();
        return .{ .micros = if (raw < 0) 0 else @intCast(raw) };
    }
...
test "audit-v4 DoH: non-zero RCODE is fail-closed" {
...
test "audit-v4 resolve-verify: strict rejects cofactored-only; cofactored accepts" {
