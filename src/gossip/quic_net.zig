...
pub const Node = struct {
    ...
    pub fn deinit(self: *Node) void {
        ...
        for (self.user_events.items) |event| event.deinit(self.allocator);
    ...
    fn flushPendingDial(self: *Node, to: NodeId) Error!void {
        ...
        while (list.items.len > 0) {
            const msg = list.orderedRemove(0);
    ...
    pub fn popUserEvent(self: *Node) ?UserEvent {
        if (self.user_events.items.len == 0) return null;
        return self.user_events.orderedRemove(0);
    }

    fn pushUserEvent(self: *Node, event: UserEvent) Error!void {
        if (self.user_events.items.len < self.max_user_events) {
            try self.user_events.ensureUnusedCapacity(self.allocator, 1);
        } else if (self.user_events.items.len > 0) {
            const dropped = self.user_events.orderedRemove(0);
            dropped.deinit(self.allocator);
