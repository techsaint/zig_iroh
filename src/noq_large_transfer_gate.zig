...
/// The gate bounds PROGRESS, not only per-op stalls. The
...
fn runLosslessTransfer(total: usize, seed: u8, alpn: [:0]const u8) !void {
    ...
    // recv buffer overflows under host load; noq recovers correctly but its (un-perf-tuned)
    // congestion controller is conservative, so a 256 MiB transfer under heavy drops can take
    // ~75s. Bounded PROGRESS model: the 180s stream timeout bounds a
