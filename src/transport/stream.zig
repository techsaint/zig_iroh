...
pub const SendImpl = struct {
    ...
    /// (borrowed slices — no intermediate re-buffering; audit-v4 #6).
...
fn sendWriter(ctx: *anyopaque) *std.Io.Writer {
    ...
    if (!send.used.load(.monotonic)) return &send.dead_writer;
...
fn sendFlush(ctx: *anyopaque) tr.Error!void {
    const send: *SendImpl = @ptrCast(@alignCast(ctx));
    if (!send.used.load(.monotonic)) return error.NotConnected;
    send.writer_storage.flush() catch return error.ConnectionLost;
}

fn sendFinish(ctx: *anyopaque) tr.Error!void {
    const send: *SendImpl = @ptrCast(@alignCast(ctx));
    if (!send.used.load(.monotonic)) return;
    // Drain staged bytes (fin=false), then queue FIN separately — the FIN is
    // a distinct command from the data drain (S4 FIN-drain separation). The
    // fin command runs even if the drain failed: a failed finish still
    // tombstones the send half (legacy parity), which doSendFin defers.
    const flush_err: ?tr.Error = blk: {
        send.writer_storage.flush() catch break :blk error.ConnectionLost;
        break :blk null;
    };
    var completion: Completion(VoidResult) = .{};
    var node: Command.Node = .{ .command = undefined };
    submit(send.endpoint, .{ .send_fin = .{ .send = send, .completion = &completion } }, &node);
    const fin_err = completion.awaitResult(send.endpoint.io_inst);
...
fn sendReset(ctx: *anyopaque) void {
    ...
    var node: Command.Node = .{ .command = undefined };
    submit(send.endpoint, .{ .send_reset = .{ .send = send, .completion = &completion } }, &node);
    completion.awaitResult(send.endpoint.io_inst);
}

pub const send_vtable: tr.SendStream.VTable = .{ .writer = sendWriter, .flush = sendFlush, .finish = sendFinish, .reset = sendReset };
...
fn recvReader(ctx: *anyopaque) *std.Io.Reader {
    ...
    if (!recv.used.load(.monotonic) or recv.stopped.load(.monotonic)) return &recv.dead_reader;
    if (!recv.ready) {
        recv.reader_storage = .{
...
fn recvStop(ctx: *anyopaque) tr.Error!void {
    ...
    var completion: Completion(VoidResult) = .{};
    var node: Command.Node = .{ .command = undefined };
    submit(recv.endpoint, .{ .recv_stop = .{ .recv = recv, .completion = &completion } }, &node);
    return completion.awaitResult(recv.endpoint.io_inst);
}

pub const recv_vtable: tr.RecvStream.VTable = .{ .reader = recvReader, .stop = recvStop };
