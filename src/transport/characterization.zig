//! Characterization suite — the G2 picoquic-backend completion driver's
//! differential oracle (transport-relay-concurrency, run 20260718T0249Z-kimi-g2-picoquic).
//!
...
//! - `greenfield` = `endpoint.zig` — the G2 edit target. It must MATCH the
//!                  legacy behavior test-for-test.
//!
//! Only wire/peer-visible and vtable-contract behavior is pinned here (GOOD-vs-
//! DRIFT pre-classification, per the kickoff): bytes, ordering, FIN/RESET
...
//! layout, sweep cadence) are deliberately NOT pinned — the G2 streams/memory
//! work is free to change those, and the change is journaled in the dossier.
//!
//! OWED-PARITY LIST (greenfield pins registered as G2 milestones land; legacy
...
//!      — FLIPPED GREEN at M1
//!   P3 `unaccepted-then-closed cnxs are reclaimed`     (audit-v4 #4 greenfield-half)
...
// contract); greenfield variants register as the G2 milestones close each gap.
// =============================================================================
...
/// endpoint keeps accepting (audit-v4 #4 — the greenfield G0/G1 scaffold had
