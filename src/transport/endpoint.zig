//! Greenfield single-owner picoquic transport endpoint (G2 — actor model).
//!
...
//! `std.Io.Mutex` — a genuine cross-task hand-off, per the C1 design
...
//! pump, tables, and concurrency model. Relay fallback stays `Disabled` here
//! (relay is the reorient-relay lane, out of G2).
...
/// `consumed_total + window` as the reader consumes (audit-v4 #6 backpressure).
...
pub const Endpoint = struct {
    ...
    socket: net.Socket,
    relay: relay_fallback.Disabled = .init(),
    ...
    pub fn initOptions(allocator: std.mem.Allocator, io: std.Io, secret: key.SecretKey, alpn: [:0]const u8, options: Options) !*Endpoint {
        ...
        // cnxs (audit-v4 #4): rapid connect/close cycles accumulate cnxs in the
    ...
    fn run(self: *Endpoint) void {
        while (!self.stopping) {
            ...
            if (self.broken) {
                ...
                _ = self.pumpIncoming(!sent) catch blk: {
                    failed = true;
                    break :blk false;
                };
                if (failed) {
                ...
                if (self.findStream(w.recv.cnx, w.recv.stream_id)) |stream| {
                    ...
                    if (stream.recvq.buffered > 0) {
                        ...
                        if (stream.recvq.buffered < stream_receive_window / 2 and !stream.fin) {
                            ...
                            // drains (audit-v4 #6 backpressure): the frame
    ...
    fn reclaimConnection(self: *Endpoint, cnx: *c.picoquic_cnx_t) void {
        for (self.streams.items) |stream| {
            ...
            // close op, so there is no safe earlier point (see the dossier's
            // F7 retention note).
    ...
    fn pumpOutgoing(self: *Endpoint) tr.Error!bool {
        ...
        while (true) {
            ...
            while (!batch.isFull()) {
                ...
                if (send_len == 0) break;
                const dest = ip4FromSockaddr(to) catch return error.ConnectionLost;
                batch.append(dest, packet[0..send_len]);
            }
            if (batch.count == 0) return sent;
            self.socket.sendMany(self.io_inst, batch.slice(), .{}) catch return error.ConnectionLost;
            sent = true;
        }
    }
    ...
    fn pumpIncoming(self: *Endpoint, wait: bool) tr.Error!bool {
        ...
        for (0..max_receive_batches_per_call) |_| {
            ...
            for (batch.messages[0..count]) |message| {
                ...
                // datagram must not kill the endpoint (audit-v4 #8 note).
                var from = sockaddrFromIp4(message.from) catch continue;
                var to = sockaddrFromIp4(self.pathLocalAddress(message.from)) catch continue;
...
test "G2: greenfield endpoint direct QUIC echo stability N=200" {
...
test "G2: greenfield recvReader survives peer stream reset without process abort" {
    // Fault injection (audit-v4 #1): the peer RESETS after handoff; reader()
...
test "G2: server close after peer close does not double-free the cnx" {
...
test "G2: parked acceptBi is failed promptly when the peer closes" {
...
test "G2: writes past the staging water-mark are wire-visible without flush" {
    ...
    // BEFORE any flush() — the audit-v4 #6 water-mark shape + iroh's
...
test "G2: concurrent callers on one endpoint are serialized by the loop" {
...
test "G2: receive-stall timeout degrades the reader and closes the conn (deferred)" {
