...
//!
//! # Ownership precondition (audit-v4 #2/#10/#19 stopgap)
//!
...
//! transfers to `endpoint.zig` until G2 enforces it structurally). It is **not**
//! a coarse take-mu-everywhere rework (throwaway at G5). See
//! `plans/transport-relay-concurrency` ownership note +
//! `docs/research/transport-ownership-reconcile-2026-07-17/SYNTHESIS.md` §4.
...
pub const Endpoint = struct {
    ...
    /// Per-connection magicsock/relay path state (VC-3 H1). Keyed by picoquic
    ...
    fn pumpRelayIncomingLocked(self: *Endpoint) tr.Error!bool {
        ...
        while (true) {
            ...
            // mid-handshake cnx. Never a shared endpoint-global slot (VC-3 H1).
            var matched = false;
            var maybe_cnx: ?*c.picoquic_cnx_t = c.picoquic_get_first_cnx(self.quic);
            while (maybe_cnx) |cnx| : (maybe_cnx = c.picoquic_get_next_cnx(cnx)) {
                const path = try self.ensurePathState(cnx);
...
fn endpointConnect(ctx: *anyopaque, peer: tr.NodeAddr) tr.Error!tr.Connection {
    ...
    // Do NOT reconfigure endpoint-global RPK here (V3-B): that raced concurrent dials.
...
// audit-v4 H1 / crypto-seam: production-path client pin oracle.
...
test "VC-1 C1: recvReader survives peer stream reset without process abort" {
...
test "VC-3 H1: two dials keep isolated magicsock path state" {
    const allocator = std.testing.allocator;
...
test "VC-3 H2: lockPump acquires under contention with bounded backoff" {
    const allocator = std.testing.allocator;
...
const connection_vtable: tr.Connection.VTable = .{ .openBi = connOpenBi, .acceptBi = connAcceptBi, .openUni = connOpenUni, .acceptUni = connAcceptUni, .remoteNodeId = connRemote, .alpn = connAlpn, .close = connClose, .io = connIo };
...
fn recvReader(ctx: *anyopaque) *std.Io.Reader {
    ...
    if (!recv.ready) {
        ...
        // process does not abort (V3-C / VC-1 C1). Callers see EndOfStream.
...
test "S4: relay fallback transfers a bi stream when direct paths are unavailable" {
    ...
    try client_relay_client.connectInPlace(io, .{ .url = relay_url, .secret_key = client_key });
    defer client_relay_client.close();
    phase.store(2, .release);
    var server_relay_client: relay_client.Client = undefined;
    try server_relay_client.connectInPlace(io, .{ .url = relay_url, .secret_key = server_key });
