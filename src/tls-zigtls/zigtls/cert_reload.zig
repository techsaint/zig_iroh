...
pub const Store = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Store {
        return initWithIo(allocator, std.Io.Threaded.global_single_threaded.io());
    }
    ...
    pub fn reloadFromFiles(self: *Store, cert_path: []const u8, key_path: []const u8) Error!u64 {
        const cert = try std.Io.Dir.cwd().readFileAlloc(self.io, cert_path, self.allocator, .limited(max_reload_file_bytes));
        errdefer self.allocator.free(cert);
        const key = try std.Io.Dir.cwd().readFileAlloc(self.io, key_path, self.allocator, .limited(max_reload_file_bytes));
