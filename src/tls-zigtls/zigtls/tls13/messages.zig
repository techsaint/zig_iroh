...
test "new session ticket decode preserves early_data extension" {
    const allocator = std.testing.allocator;
    const bytes = [_]u8{
        0x00, 0x00, 0x0e, 0x10, // lifetime
        0x11, 0x22, 0x33, 0x44, // age add
        ...
        0x00, 0x02, // ticket len
        0xbe, 0xef, // ticket
        0x00, 0x08, // extension vector len
        0x00, 0x2a, // early_data
        0x00, 0x04, // extension data len
        0xff, 0xff, 0xff, 0xff, // QUIC sentinel
    };
    var msg = try NewSessionTicketMsg.decode(allocator, &bytes);
    defer msg.deinit(allocator);

    try std.testing.expectEqual(@as(usize, 1), msg.extensions.len);
    try std.testing.expectEqual(@as(u16, 0x002a), msg.extensions[0].extension_type);
    try std.testing.expectEqualSlices(u8, &.{ 0xff, 0xff, 0xff, 0xff }, msg.extensions[0].data);
}
