...
pub const PeerValidationConfig = struct {
    ...
    /// Test-only mutation hooks (cert-validation matrix). Production callers
    /// leave these false. Each independently neuters one defense so a
    /// negative gate can prove the defense was load-bearing.
...
pub const Engine = struct {
    ...
    /// Client: the (PSK-resumption) ClientHello carried the `early_data`
    /// extension (RFC 8446 §4.2.10). Reset per ClientHello build, so a
    /// HelloRetryRequest rebuild that must drop 0-RTT reads false.
    early_data_offered: bool = false,
    /// Client: the server's EncryptedExtensions accepted our early data.
    /// Server: the ClientHello offer was accepted at CH time (PSK selected,
    /// early data enabled, replay-filter-fresh ticket). Drives the EE
    /// extension and whether the QUIC layer installs early keys at all.
    ...
    client_hello_hash: [48]u8 = [_]u8{0} ** 48,
    client_hello_hash_len: usize = 0,
    ...
    /// RFC 9000 §7.5 / RFC 8446: CRYPTO frames may split a TLS handshake
    /// message across packets (and across feeds of this function). A trailing
    /// partial message is buffered here and prepended to the next feed instead
    /// of erroring, so the reassembled stream parses message-aligned.
    pending_handshake: std.ArrayList(u8),
    ...
    pub fn deinit(self: *Engine) void {
        ...
        self.pending_handshake.deinit(self.allocator);
    }
    ...
    pub fn beginQuicClientHandshake(
        ...
        self.captureClientHelloHash();
        self.deriveEarlyTrafficSecret();
    }
    ...
    /// Client: the last ClientHello carried the early_data extension.
    pub fn earlyDataOffered(self: *const Engine) bool {
        return self.early_data_offered;
    }

    /// Client: the server accepted our 0-RTT offer (EncryptedExtensions).
    /// Server: we accepted the client's offer at ClientHello time. False on
    /// both roles until that verdict exists, so a QUIC layer polling before the
    /// flight landed never installs early keys off a stale/absent secret.
    pub fn earlyDataAccepted(self: *const Engine) bool {
        return self.early_data_accepted;
    }

    /// The resumption ticket's remembered QUIC transport parameters, when the
    /// ticket permits 0-RTT (RFC 9001 §4.6: the client restores remembered TPs
    /// for its 0-RTT flight). Null for a non-0-RTT ticket or no ticket.
    pub fn resumptionQuicTransportParameters(self: *const Engine) ?[]const u8 {
        const ticket = self.config.resumption_ticket orelse return null;
        if (!ticket.quic_0rtt_allowed) return null;
        return ticket.quic_transport_parameters;
    }
    ...
    fn ingestHandshakePayload(
        ...
        // RFC 9000 §7.5: CRYPTO frames may split a TLS handshake message across
        // packets, so a feed can end mid-message. Accumulate into the pending
        // buffer and parse only complete messages, carrying any trailing partial
        // message over to the next feed (instead of erroring).
        try self.pending_handshake.appendSlice(self.allocator, payload);
        // A validation/parse failure below aborts the handshake: drop the buffered
        // bytes so a rejected message can't corrupt the next ingest on this engine.
        // The benign trailing-partial case returns NORMALLY (via `break`, not an
        // error), so this errdefer does not fire on it — the A9 reassembly of an
        // incomplete message across feeds is preserved.
        errdefer self.pending_handshake.clearRetainingCapacity();
        var consumed: usize = 0;
        while (true) {
            const cursor = self.pending_handshake.items[consumed..];
            if (cursor.len == 0) break;
            const frame = handshake.parseOne(cursor) catch |err| switch (err) {
                // Trailing partial message — wait for the next feed.
                error.IncompleteBody => break,
                else => |e| return e,
            };
            const frame_len = 4 + @as(usize, @intCast(frame.header.length));
            ...
            if (event == .hello_retry_request) {
                if (self.config.quic_mode and self.config.role == .client) {
                    ...
                    // RFC 8446 §4.2.10: a HelloRetryRequest means the server
                    // will not process the first flight's early data — the
                    // retry ClientHello MUST NOT offer 0-RTT and the already
                    // derived early write secret is dead (the transcript
                    // changed). Kill it so the QUIC layer can't encrypt with it.
            ...
            if (self.config.role == .server and frame.header.handshake_type == .client_hello and prev_state == .start) {
                self.captureClientHelloHash();
                // The 0-RTT accept/reject decision must land BEFORE this flight
                // is built: the EncryptedExtensions in it announces it
                // (decideQuicEarlyDataAcceptance gates deriveEarlyTrafficSecret
                // below, so an unaccepted offer never yields early read keys).
                try self.decideQuicEarlyDataAcceptance(frame.body);
                self.deriveEarlyTrafficSecret();
        ...
        // Drop fully-parsed bytes; carry any trailing partial message to the
        // next feed.
        if (consumed > 0) {
            const total = self.pending_handshake.items.len;
            const remaining = total - consumed;
            std.mem.copyForwards(u8, self.pending_handshake.items[0..remaining], self.pending_handshake.items[consumed..total]);
            self.pending_handshake.shrinkRetainingCapacity(remaining);
        }
    }
    ...
    fn buildClientHelloBody(
        ...
        // Per-ClientHello state: a retry rebuild must start from "no 0-RTT
        // offer" so an HRR-downgraded flight never re-offers early data.
        self.early_data_offered = false;
        ...
        if (outbound_resumption_ticket) |ticket| {
            ...
            // RFC 8446 §4.2.10 / RFC 9001 §8: a client resuming with a
            // 0-RTT-capable ticket MAY offer early data by including an EMPTY
            // early_data extension in the ClientHello. Offer only when the
            // ticket advertises 0-RTT, this endpoint has early data enabled
            // (initChecked fail-closes `enabled` without a replay filter), and
            // this is not a HelloRetryRequest rebuild (early data is forbidden
            // after an HRR — the transcript changed). The early_data extension
            // MUST precede pre_shared_key, which RFC 8446 §4.2.11 pins as the
            // LAST ClientHello extension.
            if (ticket.quic_0rtt_allowed and
                self.config.early_data.enabled and
                !self.pending_quic_hello_retry_request)
            {
                const early = try self.allocator.alloc(u8, 0);
                try ext_list.append(self.allocator, .{
    ...
    /// Server-side 0-RTT acceptance, RFC 8446 §4.2.10 + RFC 9001 §9.2. Runs at
    /// ClientHello time because the EncryptedExtensions of the same flight must
    /// announce the verdict. Acceptance requires ALL of:
    ///   - a PSK was selected (full-handshake clients offer no early data),
    ///   - early data is enabled on this server (`initChecked` fail-closes an
    ///     enabled config that has no replay filter),
    ///   - the ClientHello carries an EMPTY early_data extension,
    ///   - the ticket identity is FRESH in the scoped replay filter — a
    ///     replayed ClientHello re-using an admitted ticket is rejected here,
    ///     which is the single-use-ticket strike register RFC 9001 §9.2
    ///     expects ("a server MAY accept 0-RTT on only one connection per
    ///     ticket").
    /// Rejection is SILENT: no alert, no failure — the handshake proceeds as a
    /// PSK resumption without 0-RTT and the QUIC layer simply never receives
    /// early read keys (rustls/quinn behave the same).
    fn decideQuicEarlyDataAcceptance(self: *Engine, client_hello_body: []const u8) EngineError!void {
        self.early_data_accepted = false;
        if (self.selected_resumption_psk == null) return;
        if (!self.config.early_data.enabled) return;
        var hello = messages.ClientHello.decode(self.allocator, client_hello_body) catch return error.InvalidHelloMessage;
        defer hello.deinit(self.allocator);
        const early = findExtensionData(hello.extensions, ext_early_data) orelse return;
        if (early.len != 0) return error.InvalidEarlyDataExtension;
        const replay_filter = self.config.early_data.replay_filter orelse return error.MissingReplayFilter;
        const binder_info = try clientHelloFirstPskBinderInfo(client_hello_body);
        const scope: early_data.ReplayScopeKey = .{
            .node_id = self.config.early_data.replay_node_id,
            .epoch = self.config.early_data.replay_epoch,
        };
        // seenOrInsertScoped returns true when the identity was ALREADY struck:
        // that is a replayed ticket flight — refuse 0-RTT, keep the handshake.
        if (replay_filter.seenOrInsertScoped(scope, binder_info.identity)) return;
    ...
    fn buildEncryptedExtensionsBody(self: *Engine) EngineError![]u8 {
        ...
        // RFC 8446 §4.2.10: acceptance is announced by an EMPTY early_data
        // extension in EncryptedExtensions (header only, 4 bytes).
        const include_early_data = self.early_data_accepted;
        if (include_early_data) ext_total_len += 4;
        ...
        if (include_early_data) {
            ...
            std.mem.writeInt(u16, out[i + 2 ..][0..2], 0, .big);
            i += 4;
        }
        if (include_alpn) {
            const protocol_len = self.negotiated_alpn_len;
    ...
    fn captureClientHelloHash(self: *Engine) void {
        switch (self.config.suite) {
            .tls_aes_128_gcm_sha256, .tls_chacha20_poly1305_sha256 => {
                const digest = self.transcriptDigestSha256();
                @memcpy(self.client_hello_hash[0..32], &digest);
                self.client_hello_hash_len = 32;
            },
            .tls_aes_256_gcm_sha384 => {
                const digest = self.transcriptDigestSha384();
                @memcpy(self.client_hello_hash[0..48], &digest);
                self.client_hello_hash_len = 48;
            },
        }
    }

    fn earlyPsk(self: *const Engine) ?TrafficSecret {
        if (self.selected_resumption_psk) |psk| return psk;
        if (self.config.resumption_ticket) |ticket| return ticket.resumption_secret;
        return null;
    }

    fn deriveEarlyTrafficSecret(self: *Engine) void {
        if (self.client_hello_hash_len == 0) return;
        // RFC 8446 §4.2.10: early traffic keys exist only for an OFFERED
        // (client) and ACCEPTED (server) 0-RTT exchange. Deriving them for a
        // plain PSK resumption without an early_data offer would hand the QUIC
        // layer keys no packet may legally use — and on the server side it
        // would decrypt 0-RTT flights the server never agreed to accept.
        if (self.config.role == .client and !self.early_data_offered) return;
        if (self.config.role == .server and !self.early_data_accepted) return;
        const psk = self.earlyPsk() orelse return;
        switch (self.config.suite) {
            .tls_aes_128_gcm_sha256 => {
                const zeros = [_]u8{0} ** 32;
                const psk_bytes = switch (psk) {
                ...
                const early = keyschedule.extract(.tls_aes_128_gcm_sha256, &zeros, &psk_bytes);
                var ch_hash: [32]u8 = undefined;
                @memcpy(&ch_hash, self.client_hello_hash[0..32]);
                const secret = keyschedule.deriveSecret(.tls_aes_128_gcm_sha256, early, "c e traffic", &ch_hash);
                if (self.config.role == .client) {
                    self.early_write_secret = .{ .sha256 = secret };
                } else {
                    self.early_read_secret = .{ .sha256 = secret };
                }
            },
            .tls_chacha20_poly1305_sha256 => {
                const zeros = [_]u8{0} ** 32;
                ...
                const early = keyschedule.extract(.tls_chacha20_poly1305_sha256, &zeros, &psk_bytes);
                var ch_hash: [32]u8 = undefined;
                @memcpy(&ch_hash, self.client_hello_hash[0..32]);
                const secret = keyschedule.deriveSecret(.tls_chacha20_poly1305_sha256, early, "c e traffic", &ch_hash);
                if (self.config.role == .client) {
                    self.early_write_secret = .{ .sha256 = secret };
                } else {
                    self.early_read_secret = .{ .sha256 = secret };
            ...
            .tls_aes_256_gcm_sha384 => {
                ...
                const early = keyschedule.extract(.tls_aes_256_gcm_sha384, &zeros, &psk_bytes);
                var ch_hash: [48]u8 = undefined;
                @memcpy(&ch_hash, self.client_hello_hash[0..48]);
                const secret = keyschedule.deriveSecret(.tls_aes_256_gcm_sha384, early, "c e traffic", &ch_hash);
                if (self.config.role == .client) {
                    self.early_write_secret = .{ .sha384 = secret };
                } else {
                    self.early_read_secret = .{ .sha384 = secret };
                }
            },
        }
    }
    ...
    fn zeroizeStagedSecrets(self: *Engine) void {
        ...
        self.zeroizeSecretSlot(&self.early_write_secret);
        std.crypto.secureZero(u8, self.client_hello_hash[0..]);
        self.client_hello_hash_len = 0;
    }
    ...
    fn validatePeerCertificatePolicy(self: *Engine, cert_msg: messages.CertificateMsg) EngineError!void {
        ...
        const now = self.nowUnix();
        if (!self.config.peer_validation.bypass_chain_verify and cert_msg.entries.len > 1) {
            var idx: usize = 0;
        ...
        if (!self.config.peer_validation.bypass_hostname_verify) {
        ...
        if (!self.config.peer_validation.bypass_trust_anchor) {
        ...
        if (!self.config.peer_validation.bypass_ocsp_check and self.config.peer_validation.enforce_ocsp) {
    ...
    fn validateHandshakeBody(self: *Engine, handshake_type: state.HandshakeType, body: []const u8) EngineError!void {
        switch (handshake_type) {
            ...
            .encrypted_extensions => {
                ...
                if (self.config.role == .client) {
                    // RFC 8446 §4.2.10: an early_data extension here ACCEPTS our
                    // offer (its absence is a rejection — the handshake carries
                    // on without 0-RTT). An acceptance for an offer we never made
                    // is illegal and aborts; the QUIC layer must never install
                    // early keys off a forged verdict.
                    if (findExtensionData(ee.extensions, ext_early_data)) |early| {
                        if (!self.early_data_offered or early.len != 0) return error.InvalidEarlyDataExtension;
                        self.early_data_accepted = true;
                    }
                }
    ...
    fn acceptClientPskOffer(self: *Engine, hello: messages.ClientHello, body: []const u8) EngineError!void {
        ...
        if (self.config.quic_mode) {
            // A resuming client MUST still carry transport parameters, but they
            // are a FRESH negotiation — not byte-identical to the ticket's
            // remembered values. The remembered TPs seed the client's 0-RTT
            // flight; the client's own parameters legitimately change every
            // connection (initial_source_connection_id, grease, and shuffled
            // parameter order are all randomized per connection, RFC 9000
            // §18.1/§18.2), so requiring byte-equality here made every real
            // 0-RTT resumption impossible. Replay defense is the strike
            // register (decideQuicEarlyDataAcceptance), not this comparison.
            if (ticket.quic_transport_parameters != null) {
                _ = findExtensionData(hello.extensions, ext_quic_transport_parameters) orelse return error.MissingQuicTransportParameters;
            }
        }
