...
pub const Connection = struct {
    ...
    fn nowNs(self: Connection) u64 {
        ...
        const io = self.config.session.io orelse return 0;
        return @intCast(std.Io.Clock.awake.now(io).toNanoseconds());
    }

    fn nowUnix(self: Connection) i64 {
        if (self.config.now_unix) |f| return f();
        return std.Io.Clock.real.now(self.config.session.io orelse return 0).toSeconds();
    }
...
fn buildClientHelloRecord(allocator: std.mem.Allocator, opts: ClientHelloBuildOptions) ![]u8 {
    ...
    const kp = std.crypto.dh.X25519.KeyPair.generate(std.testing.io);
