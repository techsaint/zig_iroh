...
//!   callers. The loop is spawned via `Io.concurrent` (not `Io.async`): under a
//!   1-CPU affinity pin, `std.Io.Threaded` defaults `async_limit` to
//!   `cpu_count - 1 = 0`, and `async` would INLINE the infinite `run` loop on
//!   the caller (measured 2026-08-09: zero UDP OutDatagrams / hang in
//!   `poll_schedule_timeout` under `taskset -c N`). `concurrent` always
//!   assigns a real worker or fails loud with `ConcurrencyUnavailable`.
//!   `std.testing.io` (Threaded) and product Threaded both support this.
...
pub const Command = union(enum) {
    ...
    send_reset: struct { send: *SendImpl, code: u64, completion: *Completion(void) },
...
///
/// On Linux, `wake_fd` is an eventfd written AFTER the mutex-linked enqueue so
/// a blocking `ppoll` in the receive loop cannot lose a command wakeup.
const Inbox = struct {
    mutex: std.Io.Mutex = .init,
    head: ?*Command.Node = null,
    tail: ?*Command.Node = null,
    wake_fd: ?posix.fd_t = null,
    ...
    fn submit(self: *Inbox, io: std.Io, node: *Command.Node) void {
        ...
        // Wake AFTER unlock so the loop never sees an empty inbox on a wake
        // that raced the link (lost-wakeup hazard (ii)).
        if (self.wake_fd) |wfd| pump.signalWake(wfd);
    }
...
pub const Endpoint = struct {
    ...
    /// Linux eventfd: wakes a blocking ppoll when a command is enqueued.
    /// null on non-Linux (portable receive uses receiveManyTimeout).
    wake_fd: ?posix.fd_t = null,
    ...
    pub fn initOptions(allocator: std.mem.Allocator, io: std.Io, secret: key.SecretKey, alpn: [:0]const u8, options: Options) !*Endpoint {
        ...
        // The fd is owned from here; `deinit` closes it on the success path,
        // but an init failure below never reaches `deinit` — close it on unwind.
        ...
        // freeServerAlpns only frees item bytes + clearRetainingCapacity; the
        // list backing store still needs deinit. Both must run on every init
        // failure after self.* is assigned (e.g. ConcurrencyUnavailable from
        // concurrent spawn after ALPN install / dead-cnx pre-reserve).
        errdefer {
            freeServerAlpns(self);
            self.server_alpns.deinit(self.allocator);
        }
        errdefer self.dead_cnxs_pending.deinit(self.allocator);
        self.gso_enabled = udp_cmsg.gsoSupported(socket.handle);
        // Command wake fd so a blocking receive cannot delay inbox commands.
        if (builtin.os.tag == .linux) {
            self.wake_fd = pump.createWakeFd() catch null;
            self.inbox.wake_fd = self.wake_fd;
        }
        // Same init-failure unwind as the socket above: `deinit` closes this
        // only on the success path. Function scope — an errdefer inside the
        // `if` block would expire with the block.
        errdefer if (self.wake_fd) |wfd| pump.closeWakeFd(wfd);
        ...
        //
        // MUST be concurrent, not async: Threaded's default async_limit is
        // cpu_count-1. On a single-CPU pin that is 0, so async inlines the
        // infinite actor loop on the caller and the endpoint deadlocks before
        // any Initial is sent. concurrent always spawns a real worker (or
        // returns ConcurrencyUnavailable fail-loud instead of hanging).
        self.loop_future = try io.concurrent(run, .{self});
        return self;
    }
    ...
    pub fn deinit(self: *Endpoint) void {
        ...
        if (self.wake_fd) |wfd| {
            pump.closeWakeFd(wfd);
            self.wake_fd = null;
            self.inbox.wake_fd = null;
        }
    ...
    pub fn pubConnect(self: *Endpoint, peer: tr.NodeAddr) tr.Error!tr.Connection {
        ...
        submit(self, .{ .connect = .{ .peer = peer, .completion = &completion } }, &node);
    ...
    pub fn pubAccept(self: *Endpoint) tr.Error!tr.Connection {
        ...
        var node: Command.Node = .{ .command = undefined };
        submit(self, .{ .accept = .{ .completion = &completion } }, &node);
        return completion.awaitResult(self.io_inst);
    }
    ...
    fn handleCommand(self: *Endpoint, command: Command) void {
        switch (command) {
            ...
            .send_reset => |cmd| {
                self.doSendReset(cmd.send, cmd.code);
                cmd.completion.complete(self.io_inst, {});
            },
    ...
    fn doOpenUni(self: *Endpoint, conn_id: u64, completion: *Completion(OpenUniResult)) void {
        ...
        const stream: tr.SendStream = .{ .impl = send };
        completion.complete(self.io_inst, stream);
    }
    ...
    fn doSendReset(self: *Endpoint, send: *SendImpl, code: u64) void {
        if (!send.used.load(.monotonic)) return;
        // local_stream_error is the application error code on RESET_STREAM.
        if (c.picoquic_reset_stream(send.cnx, send.stream_id, code) == 0) {
            _ = self.pumpOutgoing() catch {};
        }
    ...
    fn closeConnImpl(self: *Endpoint, conn: *ConnectionImpl) void {
        ...
        // delete — matching the legacy backend's teardown order. Both engine
        // calls are presence-guarded: for a SERVER cnx the engine's prepare
        // pass can return DISCONNECTED mid-pump and auto-delete the cnx
        // (sender.c:3806-3815) — and any earlier engine-side death likewise
        // frees it before this command lands. The unguarded close/delete were
        // a use-after-free (GPF in picoquic_delete_cnx at the perflog call,
        // observed 2026-08-08 in the G2 receive-stall test; the engine-side
        // delete already ran the close callback → noteCnxDead there).
        if (self.cnxPresent(cnx)) {
    ...
    /// callback would otherwise re-queue the cnx for deletion). Also forgets
    /// the RPK verifier's per-cnx entry: the suppressed noteCnxDead is the
    /// only other forget site, so without this every guarded delete leaks an
    /// entry keyed on the raw cnx pointer — and a future cnx at the same
    /// recycled address could inherit a stale `expected` key and reject a
    /// legitimate handshake (rpk.c verify_raw_pubkey).
    ...
    /// Engine-still-holds-this-cnx check. Loop-owned and synchronous, so
    /// nothing creates or frees cnxs between the check and its immediate
    /// caller's engine call — an absent result means the engine already tore
    /// the cnx down (its delete-time callback ran noteCnxDead with
    /// `deleting_cnx` unset).
    fn cnxPresent(self: *Endpoint, cnx: *c.picoquic_cnx_t) bool {
        var maybe_cnx: ?*c.picoquic_cnx_t = c.picoquic_get_first_cnx(self.quic);
        while (maybe_cnx) |current| : (maybe_cnx = c.picoquic_get_next_cnx(current)) {
            if (current == cnx) return true;
        }
        return false;
    }
    ...
    fn pumpOutgoing(self: *Endpoint) tr.Error!bool {
        var sent = false;
        while (true) {
            // `.{}` compiles to a memset of the whole (~12 KiB) batch per
            // iteration; only `count` gates which slots are read (append
            // writes a slot before it can be sliced), so init just that.
            var batch: pump.OutgoingBatch = undefined;
            batch.count = 0;
            while (!batch.isFull()) {
    ...
    fn sendUdpBatch(self: *Endpoint, batch: []net.OutgoingMessage) tr.Error!void {
        if (self.gso_enabled and batch.len > 1) {
            const seg_size = batch[0].data_len;
            var uniform = true;
            for (batch[1..]) |msg| {
                if (msg.data_len != seg_size) {
                    uniform = false;
                    break;
                }
            }
            if (uniform and seg_size > 0) {
                var gso_buf: [pump.outgoing_batch_size * c.PICOQUIC_MAX_PACKET_SIZE]u8 = undefined;
                var offset: usize = 0;
                for (batch) |msg| {
                    @memcpy(gso_buf[offset..][0..msg.data_len], msg.data_ptr[0..msg.data_len]);
                    offset += msg.data_len;
                }
                var control_buf: [udp_cmsg.send_control_space]u8 = undefined;
                var enc = udp_cmsg.Encoder.init(&control_buf);
                enc.pushSegmentSize(@intCast(seg_size)) catch {
                ...
                const dest = batch[0].address;
                udp_cmsg.rawSendmsg(self.socket.handle, dest, gso_buf[0..offset], enc.finish(), true) catch |err| switch (err) {
                    error.GsoRejected => {
                        self.gso_enabled = false;
                        self.socket.sendMany(self.io_inst, batch, .{}) catch |e2| switch (e2) {
                            error.AddressFamilyUnsupported => return,
                            else => return error.ConnectionLost,
                        };
                    },
    ...
    fn pumpIncoming(self: *Endpoint, wait: bool) tr.Error!bool {
        return switch (builtin.os.tag) {
            .linux => self.pumpIncomingLinux(wait),
            else => self.pumpIncomingPortable(wait),
        };
    }

    /// Linux event-driven receive: recvmmsg(DONTWAIT) first; on EAGAIN, one
    /// `ppoll({socket, wake_fd})` only when picoquic has a positive wake delay
    /// and the caller allowed waiting. Never zero-timeout busy-polls.
    fn pumpIncomingLinux(self: *Endpoint, wait: bool) tr.Error!bool {
        var progressed = false;
        var batch: pump.IncomingBatch = undefined;
        for (0..max_receive_batches_per_call) |_| {
            batch.init();
            // (a) non-blocking recvmmsg batch
            var count = pump.recvmmsgDontWait(self.socket.handle, &batch, parseIncomingFrom) catch
                return error.ConnectionLost;
            if (count == 0) {
                if (progressed or !wait) break;
                // (b) wake_delay ≤ 0 ⇒ return immediately (outgoing/timer work)
                const wake_delay_us = c.picoquic_get_next_wake_delay(
                    self.quic,
                    c.picoquic_current_time(),
                    @intCast(pump.max_wait_ns / std.time.ns_per_us),
                );
                const wait_ns = pump.waitNsFromWakeDelayUs(wake_delay_us) orelse break;
                // (c) ONE blocking ppoll on socket + command wake fd
                const ready = pump.waitSocketOrWake(self.socket.handle, self.wake_fd, wait_ns) catch
                    return error.ConnectionLost;
                if (ready.wake_ready) {
                    if (self.wake_fd) |wfd| pump.drainWake(wfd);
                    // Command pending — exit so run() can drainInbox promptly.
                    break;
                }
                if (!ready.socket_ready) break;
                count = pump.recvmmsgDontWait(self.socket.handle, &batch, parseIncomingFrom) catch
                    return error.ConnectionLost;
                if (count == 0) break;
            }
            try self.ingestIncomingBatch(batch.messages[0..count]);
            progressed = true;
        }
        return progressed;
    }

    fn pumpIncomingPortable(self: *Endpoint, wait: bool) tr.Error!bool {
        var progressed = false;
        var batch: pump.IncomingBatch = undefined;
        for (0..max_receive_batches_per_call) |_| {
            batch.init();
    ...
    fn ingestIncomingBatch(self: *Endpoint, messages: []const net.IncomingMessage) tr.Error!void {
    ...
    fn parseIncomingFrom(storage: *const c.struct_sockaddr_storage) error{ConnectionLost}!net.IpAddress {
        return ipFromSockaddr(storage.*) catch error.ConnectionLost;
    }
...
pub const ConnectionImpl = struct {
    ...
    pub fn pubOpenBi(self: *ConnectionImpl) tr.Error!tr.BiStream {
        ...
        var node: Command.Node = .{ .command = undefined };
        submit(self.endpoint, .{ .open_bi = .{ .conn_id = self.id, .completion = &completion } }, &node);
    ...
    pub fn pubAcceptBi(self: *ConnectionImpl) tr.Error!tr.BiStream {
        ...
        var node: Command.Node = .{ .command = undefined };
        submit(self.endpoint, .{ .accept_bi = .{ .conn_id = self.id, .completion = &completion } }, &node);
        return completion.awaitResult(self.endpoint.io_inst);
    }
    pub fn pubOpenUni(self: *ConnectionImpl) tr.Error!tr.SendStream {
        var completion: Completion(OpenUniResult) = .{};
        var node: Command.Node = .{ .command = undefined };
        submit(self.endpoint, .{ .open_uni = .{ .conn_id = self.id, .completion = &completion } }, &node);
        return completion.awaitResult(self.endpoint.io_inst);
    }
    pub fn pubAcceptUni(self: *ConnectionImpl) tr.Error!tr.RecvStream {
        var completion: Completion(AcceptUniResult) = .{};
        var node: Command.Node = .{ .command = undefined };
        submit(self.endpoint, .{ .accept_uni = .{ .conn_id = self.id, .completion = &completion } }, &node);
        return completion.awaitResult(self.endpoint.io_inst);
    }
    ...
    pub fn pubRemoteAddress(self: *ConnectionImpl) ?net.IpAddress {
        ...
        c.picoquic_get_peer_addr(self.cnx, &sa);
    ...
    pub fn pubClose(self: *ConnectionImpl) void {
        var completion: Completion(void) = .{};
        var node: Command.Node = .{ .command = undefined };
        submit(self.endpoint, .{ .close_conn = .{ .conn_id = self.id, .completion = &completion } }, &node);
        completion.awaitResult(self.endpoint.io_inst);
    }
    pub fn pubIo(self: *ConnectionImpl) std.Io {
        return self.endpoint.io_inst;
    }
    pub fn pubStats(_: *ConnectionImpl) tr.ConnectionStats {
        return .{};
    }
};
...
test "server close after peer close does not double-free the cnx" {
    ...
    // new opens), i.e. the close callback + death sweep ran. The peer's
    // CONNECTION_CLOSE is async end-to-end: while the close datagram is still
    // in flight an open may legitimately succeed — the loop drains its inbox
    // commands BEFORE the receive pump in the same iteration, so an openBi
    // command submitted after close() can still be handled ahead of the
    // already-queued close datagram. Poll-with-deadline for NotConnected
    // instead of one-shot-failing a transient open (the 2026-08-08 G2 flake:
    // a one-shot assert racing close observation — same family as the
    // accept-path poll-with-timeout hardening).
    ...
    if (!death_observed) return error.TestExpectedRemoteClosed;

    // The gate is monotonic: once the death is observed, opens STAY denied.
    if (server_conn.openBi()) |_| {
...
test "greenfield Endpoint deinit zeroizes stored secret without touching caller copy" {
    const backing = try std.heap.page_allocator.alloc(u8, @sizeOf(Endpoint) + (1 << 20));
    ...
    const endpoint = try Endpoint.init(allocator, std.testing.io, caller_secret, "iroh-g0-zeroize-deinit");
...
test "greenfield Endpoint init-failure zeroizes stored secret without touching caller copy" {
    const backing = try std.heap.page_allocator.alloc(u8, @sizeOf(Endpoint) + (1 << 20));
    defer std.heap.page_allocator.free(backing);
    var fba = std.heap.FixedBufferAllocator.init(backing);
    const allocator = fba.allocator();
...
test "greenfield Endpoint init-failure closes the bound socket and wake fd" {
    ...
    const fds_before = try countOpenFds(std.testing.io);
    const seed = [_]u8{0x5a} ** 32;
    const caller_secret = key.SecretKey.fromBytes(seed);
    const oversized_alpn: [:0]const u8 = "x" ** 65;
    // Fails in replaceServerAlpns, after the socket is bound and the wake fd
    // is created: without the init-failure errdefers both fds leak.
    const result = Endpoint.initOptions(allocator, std.testing.io, caller_secret, oversized_alpn, .{});
    try std.testing.expectError(error.InvalidAlpn, result);
    try std.testing.expectEqual(fds_before, try countOpenFds(std.testing.io));
}

// Regression guard for the single-CPU deadlock (2026-08-09):
// `std.Io.Threaded` defaults `async_limit` to `cpu_count - 1`. Under a 1-CPU
// affinity pin that is 0 (`.nothing`), so `io.async(run)` inlines the infinite
// actor loop on the caller and the endpoint never sends an Initial. Spawning
// via `io.concurrent` keeps a real worker even when `async_limit` is empty.
test "G2: actor loop spawns under Threaded async_limit=.nothing (1-CPU pin model)" {
    if (builtin.single_threaded) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    var threaded = std.Io.Threaded.init(allocator, .{
        .async_limit = .nothing, // models taskset -c N / nproc=1
    });
    ...
    const client_key = key.SecretKey.fromBytes([_]u8{0xE1} ** 32);
    const server_key = key.SecretKey.fromBytes([_]u8{0xE2} ** 32);
    const alpn: [:0]const u8 = "iroh-g2-single-cpu";
    ...
    // Wire-observable handshake + echo: proves the actor loops ran concurrently
    // with the caller's connect/accept even though async would have inlined.
    try echoRound(client_ep, server_ep, server_key, 0);
}

test "G2: Endpoint.init fails loud when concurrent workers are unavailable" {
    if (builtin.single_threaded) return error.SkipZigTest;
    const allocator = std.testing.allocator;
    var threaded = std.Io.Threaded.init(allocator, .{
        .async_limit = .nothing,
    ...
    const secret = key.SecretKey.fromBytes([_]u8{0xE3} ** 32);
    try std.testing.expectError(
        error.ConcurrencyUnavailable,
        Endpoint.init(allocator, io, secret, "iroh-g2-no-workers"),
    );
}
