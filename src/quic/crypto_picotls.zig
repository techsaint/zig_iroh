...
//!
//! API touchpoints are pinned to `dependancies/picotls/include/picotls.h`:
...
// Shared handshake types live in `crypto.zig`; alias them so the moved body
// reads identically to the pre-split source.
const Config = crypto.Config;
const Role = crypto.Role;
const Epoch = crypto.Epoch;
const Direction = crypto.Direction;
const TrafficSecret = crypto.TrafficSecret;
const HandshakeOutput = crypto.HandshakeOutput;
const max_epoch = crypto.max_epoch;
const max_secret_len = crypto.max_secret_len;
/// TLS extension id for `quic_transport_parameters` (RFC 9001 §8.2 / RFC 9000 §18).
const quic_transport_parameters_ext = crypto.quic_transport_parameters_ext;
