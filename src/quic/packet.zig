...
pub fn decodeProtectedLongHeader(bytes: []const u8) !ProtectedLongHeader {
    var cursor: coding.Cursor = .{ .bytes = bytes };
    const first = try cursor.readU8();
    if (first & fixed_bit == 0) return error.FixedBitUnset;
...
pub fn decodeProtectedHeader(bytes: []const u8) !ProtectedHeader {
    const decoded = try decodeProtectedLongHeader(bytes);
...
/// Build a RFC 9000 Version Negotiation packet (long header, version=0, fixed bit unset).
pub fn buildVersionNegotiation(
    ...
    var index: usize = 0;
    try coding.writeU8(long_header_form, out, &index); // fixed bit clear
...
pub fn parseVersionNegotiation(bytes: []const u8) !VersionNegotiation {
    ...
    if ((bytes[0] & fixed_bit) != 0) return error.VersionNegotiationMalformed;
    var cursor: coding.Cursor = .{ .bytes = bytes };
    _ = try cursor.readU8();
    const version = try cursor.readU32();
    ...
    var versions: [16]u32 = undefined;
    if (count > versions.len) return error.VersionNegotiationMalformed;
    var i: usize = 0;
    while (i < count) : (i += 1) {
        versions[i] = try cursor.readU32();
    }
    return .{
        .dst_cid = dst_cid,
        .src_cid = src_cid,
        .supported_versions = versions[0..count],
    };
}
...
test "noq initial protected header round trips invariant fields" {
    ...
    const decoded = try decodeProtectedHeader(encoded);
...
test "0-RTT protected long header skeleton decodes without token" {
    ...
    const decoded = try decodeProtectedLongHeader(buf[0..index]);
    ...
    try std.testing.expectError(error.NonInitialUnsupported, decodeProtectedHeader(buf[0..index]));
}
...
test "N-3 version negotiation build/parse roundtrip" {
    ...
    const built = try buildVersionNegotiation(allocator, dst, src, &supported);
    defer allocator.free(built);
    try std.testing.expectEqual(@as(u8, 0x80), built[0]);
    ...
    try std.testing.expectEqual(@as(usize, 2), parsed.supported_versions.len);
