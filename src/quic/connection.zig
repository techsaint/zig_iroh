...
/// Largest UDP datagram the engine builds or accepts (F19). noq's default
/// `max_udp_payload_size` is 1472 (1500 − 28 for IPv4); PMTUD probes within
/// this envelope, and the advertised TP is clamped to it in create().
...
const TlsSession = union(enum) {
    ...
    /// B8: the TLS alert code of the last handshake failure, if the backend
    /// surfaced one (noq `rustls::Connection::alert`, crypto/rustls.rs:100).
    pub fn lastAlertCode(self: *const TlsSession) ?u8 {
        return switch (self.*) {
            .picotls => |tls| picotls: {
                if (comptime crypto.picotls_enabled) {
                    break :picotls tls.lastAlertCode();
                } else {
                    break :picotls null;
                }
            },
            .zigtls => |tls| zigtls: {
                if (comptime crypto.zigtls_enabled) {
                    break :zigtls tls.lastAlertCode();
                } else {
                    break :zigtls null;
                }
            },
    ...
    fn exportSecret(self: *TlsSession, label: [:0]const u8, context_value: []const u8, out: []u8) !void {
        return switch (self.*) {
            .picotls => |tls| picotls: {
                if (comptime crypto.picotls_enabled) {
                    break :picotls tls.exportSecret(label, context_value, out);
                } else {
                    break :picotls error.PicotlsError;
                }
            },
            .zigtls => |tls| zigtls: {
                if (comptime crypto.zigtls_enabled) {
                    break :zigtls tls.exportSecret(label, context_value, out);
                } else {
                    break :zigtls error.ZigtlsDisabled;
                }
            },
    ...
    fn serverName(self: *const TlsSession) ?[]const u8 {
        return switch (self.*) {
            .picotls => |tls| picotls: {
                if (comptime crypto.picotls_enabled) {
                    break :picotls tls.serverName();
                } else {
                    break :picotls null;
                }
            },
...
pub const Connection = struct {
    ...
    pub fn pollTransmit(self: *Connection, now: Instant) Error!?Transmit {
        ...
        spaces: while (true) {
            ...
            if (space == .data) {
                ...
                if (!(self.state == .established or self.handshake_confirmed or self.close_frame != null or self.drain_close_pending)) break :spaces;
                ...
                if (have_packet) {
                ...
                } else if (self.takeHintedProbe()) |probe| {
                    ...
                    var tx = try self.finishPacket(.data, keys, &probe_frames, &probe_content, 0, true, pn, min_client_initial_datagram_size, 0, 0);
    ...
    fn installKeysFromTls(self: *Connection) !void {
        ...
        if (self.tls.isComplete() and self.state == .handshake) {
            ...
            if (self.write_keys[@intFromEnum(spaces.SpaceId.data)] != null) {
                ...
                if (self.role == .server) self.discardSpaceKeys(.handshake);
            }
        }
    ...
    fn buildSpacePacket(self: *Connection, space: spaces.SpaceId, wire_cap: usize, initial_pad: usize, extra_min_size: usize) Error!?Transmit {
        ...
        if (self.state == .closed or self.state == .draining) {
            ...
            const tx = self.finishPacket(space, keys, frames[0..n], &content, cn, ack_eliciting, pn, extra_min_size, 0, close_pad) catch |err| switch (err) {
        ...
        if (space == .data and self.state == .established) {
            ...
            if (self.new_token_pending and self.new_token_override.len != 0 and n < frames.len) {
            ...
            // clear of it.
            if (self.shouldSendAckFrequency()) self.ack_frequency_pending = true;
            ...
            if (self.pending_immediate_ack and n < frames.len) {
        ...
        if (space == .data and self.state == .established and n == 0) {
            if (self.probe_mtu) |target| {
                if (self.probe_pn == null) {
                    ...
                    ack_eliciting = true;
                    const pn = self.allocTxNumber(space);
                    return self.finishPacket(space, keys, frames[0..n], &content, cn, ack_eliciting, pn, target, 0, initial_pad) catch |err| switch (err) {
        ...
        const tx = self.finishPacket(space, keys, frames[0..n], &content, cn, ack_eliciting, pn, @max(min_size, extra_min_size), paced_payload_bytes, initial_pad) catch |err| switch (err) {
    ...
    fn finishPacket(
        ...
        const remaining_budget = self.remainingPacketBudget(space);
        if (space == .data) {
        ...
        self.sent_with_keys[@intFromEnum(space)] +|= 1;
    ...
    fn handleLongPacket(self: *Connection, data: []const u8) Error!usize {
        ...
        const keys = self.read_keys[@intFromEnum(space)] orelse {
    ...
    fn feedTls(self: *Connection, space: spaces.SpaceId, data: []const u8) Error!void {
        ...
        var out = self.tls.handleMessage(self.allocator, epoch, data) catch |err| {
        ...
        // completion when neither exists. Not per CRYPTO feed.
...
test "DATAGRAM max size reports payload bytes after frame overhead" {
    ...
    const queued = pair.client.datagram_out.popFront() orelse return error.UnexpectedState;
...
test "inbound DATAGRAM without local support protocol-closes" {
    ...
    try pair.client.processPayload(.data, 9004, enc);
...
test "inbound DATAGRAM over local frame limit protocol-closes" {
    ...
    var buf: [16]u8 = undefined;
    const enc = try (frame.Frame{ .datagram = .{ .data = "x", .with_length = true } }).encode(&buf);
    try pair.client.processPayload(.data, 9005, enc);
...
test "A7: HANDSHAKE_DONE from a client protocol-closes the server" {
    ...
    try pair.server.processPayload(.data, 9101, enc);
...
test "A7: HANDSHAKE_DONE from the server confirms the client handshake (control)" {
    ...
    var buf: [8]u8 = undefined;
    const enc = try (frame.Frame{ .handshake_done = {} }).encode(&buf);
    try pair.client.processPayload(.data, 9102, enc);
...
fn expectIllegalFrameInSpace(space: spaces.SpaceId, f: frame.Frame, pn: u64) !void {
    ...
    const enc = try f.encode(&buf);
    try pair.server.processPayload(space, pn, enc);
    const close_frame = pair.server.close_frame orelse return error.UnexpectedState;
    try std.testing.expectEqual(err_protocol_violation, close_frame.error_code);
}
...
test "A8: allow-list frames stay legal in every space (control)" {
    ...
    for ([_]spaces.SpaceId{ .initial, .handshake, .data }, 0..) |space, i| {
        ...
        try pair.server.processPayload(space, 9121 + @as(u64, @intCast(i)), enc);
        try std.testing.expect(pair.server.close_frame == null);
    }
    ...
    try pair.server.processPayload(.data, 9131, enc);
    try std.testing.expect(pair.server.close_frame == null);
}
...
test "crafted ACK with underflowing ranges protocol-closes FRAME_ENCODING_ERROR" {
    ...
    try std.testing.expectError(
        ...
        pair.client.processPayload(.data, 9201, &.{ 0x02, 0x03, 0x00, 0x00, 0x04 }),
    );
    const close_frame = pair.client.close_frame orelse return error.UnexpectedState;
    try std.testing.expectEqual(err_frame_encoding, close_frame.error_code);
    ...
    try std.testing.expectError(
        error.FrameEncodeFailed,
        pair2.client.processPayload(.data, 9202, &.{ 0x02, 0x05, 0x00, 0x01, 0x01, 0x04, 0x00 }),
    );
...
test "G3: empty packet payload is a PROTOCOL_VIOLATION (noq Iter::new)" {
    var pair = try makePair(std.testing.allocator, null);
    defer pair.deinit();
    try std.testing.expectError(error.EmptyPacket, pair.client.processPayload(.data, 9301, &.{}));
    const close_frame = pair.client.close_frame orelse return error.UnexpectedState;
    try std.testing.expectEqual(err_protocol_violation, close_frame.error_code);
}
...
test "G3: padding-only payload is NOT a violation (control)" {
    ...
    var pair = try makePair(std.testing.allocator, null);
    defer pair.deinit();
    try pair.client.processPayload(.data, 9302, &.{ 0x00, 0x00, 0x00 });
    try std.testing.expect(pair.client.close_frame == null);
}
...
test "G5: outgoing ACK Delay encodes (now - largest recv time) >> our exponent" {
    ...
    try pair.server.processPayload(.data, 9401, enc);
    ...
    pair.server.now = 10_000_000_000;
    try pair.server.processPayload(.handshake, 9402, enc);
...
test "G5: OUR advertised ack_delay_exponent scales the field (control)" {
    ...
    pair.server.peer_ack_eliciting_threshold = null;
    pair.server.now = 10_000_000_000;
    try pair.server.processPayload(.data, 9411, enc);
...
test "G11: ACK_FREQUENCY reordering_threshold is stored (noq set_ack_frequency_params)" {
    ...
    for ([_]u64{ 0, 7 }, 0..) |reorder, i| {
        ...
        try pair.client.processPayload(.data, 9600 + @as(u64, @intCast(i)), af_enc);
...
test "G11: largest ack-eliciting PN is tracked per space (is_out_of_order plumbing)" {
    ...
    try pair.client.processPayload(.data, pn, ping_enc);
    ...
    pn += 1;
    try pair.client.processPayload(.data, pn, &.{ 0x00, 0x00 });
...
test "F13: OBSERVED_ADDR frames flow only within the negotiated roles" {
    ...
    try plain.client.processPayload(.data, 9001, try f.encode(&buf));
...
test "IPv6 n0 NAT frames decode + surface (no UnsupportedFrameType close), codec round-trips" {
    ...
    try pair.client.processPayload(.data, 9201, try f.encode(&buf));
    try std.testing.expect(pair.client.state == .established);
    var got6 = false;
    while (pair.client.poll()) |ev| {
        switch (ev) {
    ...
    try pair.client.processPayload(.data, 9202, try r.encode(&buf));
    try std.testing.expect(pair.client.state == .established);
}
...
test "OBSERVED_ADDR reports are seq-gated (stale and duplicate reports drop)" {
    ...
    try pair.client.processPayload(.data, 9101, try f5.encode(&buf));
    try pair.client.processPayload(.data, 9102, try f3.encode(&buf)); // older: drop
    try pair.client.processPayload(.data, 9103, try f5b.encode(&buf)); // equal: drop
    try pair.client.processPayload(.data, 9104, try f6.encode(&buf)); // newer: surface
...
test "A9: out-of-order CRYPTO is buffered and reassembled in order" {
    ...
    const tail = frame.Frame{ .crypto = .{ .offset = half, .data = bytes[half..] } };
    try pair.client.processPayload(.initial, 100, try tail.encode(&buf));
    ...
    try pair.client.processPayload(.initial, 101, try tail.encode(&buf));
    try std.testing.expectEqual(@as(u64, 0), pair.client.crypto_in_offset[si]);
    try std.testing.expectEqual(@as(usize, 1), pair.client.crypto_in_pending[si].items.len);

    // Head: feeds the head, then drains the buffered tail contiguously.
    const head = frame.Frame{ .crypto = .{ .offset = 0, .data = bytes[0..half] } };
    try pair.client.processPayload(.initial, 102, try head.encode(&buf));
...
test "A9: CRYPTO past the 16 KiB buffer bound is CRYPTO_BUFFER_EXCEEDED" {
    ...
    try pair.client.processPayload(.initial, 100, try at_bound.encode(&buf));
    try std.testing.expect(pair.client.close_frame == null);
    try std.testing.expectEqual(@as(usize, 1), pair.client.crypto_in_pending[si].items.len);
    ...
    try pair.client.processPayload(.initial, 101, try past.encode(&buf));
    try std.testing.expect(pair.client.close_frame != null);
...
test "A9: new CRYPTO data at a stale level is PROTOCOL_VIOLATION" {
    ...
    try pair.client.processPayload(.initial, 9000, try old.encode(&buf));
    try std.testing.expect(pair.client.close_frame == null);
    ...
    try pair.client.processPayload(.initial, 9001, try stale.encode(&buf));
    try std.testing.expect(pair.client.close_frame != null);
    try std.testing.expectEqual(err_protocol_violation, pair.client.close_frame.?.error_code);
}
...
test "G4: default cadence defers the first in-order packet, ACKs the second" {
    ...
    try s.processPayload(.data, 5001, ping);
    try std.testing.expect(s.needs_ack[si]);
    try std.testing.expect(s.ackDeadlineForTest(.data) == null);
}
...
test "G4: out-of-order packet forces an immediate ACK below the threshold" {
    ...
    var buf: [8]u8 = undefined;
    const ping = try (frame.Frame{ .ping = {} }).encode(&buf);
    try s.processPayload(.data, 5001, ping); // 5000 never arrives
    try std.testing.expect(s.needs_ack[si]);
    // And the same for a BELOW-previous-largest arrival (spaces.rs:1226).
    s.needs_ack[si] = false;
    s.peer_ack_eliciting_pending = 0;
    try s.processPayload(.data, 5000, ping); // fills the hole, behind the tracker
    try std.testing.expect(s.needs_ack[si]);
}
...
test "G4: reordering_threshold 0 disables the out-of-order immediate ACK" {
    ...
    var buf: [8]u8 = undefined;
    const ping = try (frame.Frame{ .ping = {} }).encode(&buf);
    try s.processPayload(.data, 5001, ping); // gapped, but the peer opted out
    try std.testing.expect(!s.needs_ack[si]);
    try std.testing.expect(s.ackDeadlineForTest(.data) != null);
}
...
test "G4: draft-§6.1 reordering_threshold > 1 uses the largest_acked tracker" {
    ...
    var buf: [8]u8 = undefined;
    const ping = try (frame.Frame{ .ping = {} }).encode(&buf);
    try s.processPayload(.data, 102, ping); // hole at 101; 102-101 = 1 < 3
    try std.testing.expect(!s.needs_ack[si]);
    try s.processPayload(.data, 103, ping); // 103-101 = 2 < 3
    try std.testing.expect(!s.needs_ack[si]);
    try s.processPayload(.data, 104, ping); // 104-101 = 3 >= 3 → immediate
    try std.testing.expect(s.needs_ack[si]);
}
...
test "G4: emitting the ACK restarts the threshold count and records largest_acked (noq acks_sent)" {
    ...
    var buf: [8]u8 = undefined;
    const ping = try (frame.Frame{ .ping = {} }).encode(&buf);
    try s.processPayload(.data, 5001, ping); // out-of-order → immediate
    try std.testing.expect(s.needs_ack[si]);
    var frames: [16]frame.Frame = undefined;
    var n: usize = 0;
