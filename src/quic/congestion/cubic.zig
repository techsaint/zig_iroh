//! CUBIC congestion controller (RFC 8312 / iroh default) — N3b-4.
...
pub const Cubic = struct {
    ...
    fn onCongestionEvent(self: *Cubic, now: Instant, sent: Instant, is_persistent: bool, _: bool, _: u64, _: u64) void {
        if (self.recovery_start_time) |rs| {
            if (sent <= rs) return;
        }
