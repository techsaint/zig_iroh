 ...
 *  Mirrors original/iroh/iroh/src/tls/name.rs::decode. Returns 0 on success.
    ...
    size_t num_certs)
{
     ...
     * a client pin (audit-v4 H1 finding 3). */
    if (cnx == NULL) {
    ...
    /* Per-cnx expected peer (V3-B): concurrent dials must not share one endpoint-global key. */
    iroh_rpk_cnx_peer_t* entry = find_cnx_peer(self, cnx);
    ...
    int require_client_authentication)
{
    ...
    /* expected_peer_public_key is ignored here (V3-B): per-cnx expectations are
