...
/// Set by the ACME provisioning thread when certificate acquisition fails at
/// startup; main exits nonzero after the shutdown it triggers. Startup
/// provisioning failure is fatal (fail closed): a relay that cannot obtain
/// the certs it was configured to serve must not keep running certless.
var g_acme_provision_failed: std.atomic.Value(bool) = .init(false);

/// Runs initial ACME provisioning off the accept path: the ACME validation
/// authority calls BACK into this relay's listener during TLS-ALPN-01, so
/// acquisition must overlap the accept loop. On success starts the renewal
/// thread; on failure requests shutdown and flags main to exit nonzero.
fn provisionAcmeCerts(manager: *relay.acme.Manager, server: *Server) void {
    manager.ensureCertificates() catch |err| {
        std.debug.print("relay: acme certificate provisioning failed: {}\n", .{err});
        g_acme_provision_failed.store(true, .release);
        server.initiateShutdown();
        return;
    };
    manager.startRenewal() catch |err| {
        std.debug.print("relay: acme renewal thread failed to start: {}\n", .{err});
        g_acme_provision_failed.store(true, .release);
        server.initiateShutdown();
        return;
    };
    std.debug.print("relay: acme certificates ready\n", .{});
}
...
/// periods without burning CPU. Also exits when the server stops on its own
/// (programmatic shutdown, e.g. failed ACME provisioning) so the deferred
/// join in main cannot hang the process.
fn shutdownWatcher(server: *Server) void {
    while (g_shutdown_signal.load(.acquire) == 0 and server.running.load(.acquire)) {
        server.io.sleep(std.Io.Duration.fromMilliseconds(20), .awake) catch {};
    }
    if (g_shutdown_signal.load(.acquire) != 0) server.initiateShutdown();
}
...
pub fn main(init: std.process.Init) !void {
    ...
    // --- ACME certificate manager (optional) --------------------------------
    // Created before the server so its cert selector can be borrowed by
    // ServerConfig; the manager outlives the server via defer order (its
    // deinit runs after server.deinit below).
    var acme_manager: ?*relay.acme.Manager = null;
    defer if (acme_manager) |m| m.deinit();
    if (config.acme) |acme_cfg| {
        acme_manager = relay.acme.Manager.init(arena, io, .{
            .directory_url = acme_cfg.directory_url.?,
            .ca_pem_path = acme_cfg.ca_pem_path.?,
            .account_key_path = acme_cfg.account_key_path.?,
            .cert_dir = acme_cfg.cert_dir.?,
            .hostnames = acme_cfg.hostnames,
            .contact = acme_cfg.contact,
        }) catch |err|
            return fatal(out, "cannot initialize ACME certificate manager", @errorName(err));
    }
    ...
    var server_config: ServerConfig = .{
        ...
        .tls_cert_selector = if (acme_manager) |m| m.certSelector() else null,
    ...
    // ACME provisioning overlaps the accept loop: the CA's validation
    // authority calls back into THIS listener during TLS-ALPN-01. Joined
    // after the accept loop exits (it requests shutdown on failure).
    var provision_thread: ?std.Thread = null;
    if (acme_manager) |m| {
        provision_thread = std.Thread.spawn(.{}, provisionAcmeCerts, .{ m, &server }) catch
            return fatal(out, "cannot start ACME provisioning thread", null);
    }

    const scheme: []const u8 = if (config.tls_cert_path != null or acme_manager != null) "wss" else "ws";
    ...
    if (qad_server) |qs| qs.initiateShutdown();
    server.deinit();
    if (g_acme_provision_failed.load(.acquire))
        return fatal(out, "ACME certificate provisioning failed", null);
    try out.writeAll("relay: stopped\n");
}
