...
implements the core connection, discovery, relay, and transfer surfaces. there are quite a few gaps but it's working!

- **Ports iroh at:** `255a939b1a` (`v1.0.0-2-g255a939b1a`)

- **Toolchain:** Zig `0.16.0`

- **Status:** `1.0.0-zig-alpha2` / package `0.2.0-alpha.2` — subset alpha
...
Latest committed stack-self `ReleaseFast` measurements. Each row is shown as a ratio to a Rust iroh reference measured on the **same host** (the cross-product-comparable figure) and as raw goodput (which is *not* comparable across hosts):

- `picoquic-picotls` (mature C QUIC + C TLS): **1.580x** iroh — 275.48 Mbps, measured 2026-07-15

- `noq-picotls` (native Zig QUIC + C TLS): **2.018x** iroh — 2244.33 Mbps, measured 2026-08-07

- `noq-zigtls` (full native Zig QUIC + TLS): **2.008x** iroh — 2232.81 Mbps, measured 2026-08-07

`picoquic-picotls` was measured on a different, slower host than the `noq` products, so its lower raw Mbps mostly reflects the hardware, not the stack — compare the ratios, not the megabits.

Full throughput detail, provenance, and caveats: [`docs/PERFORMANCE.md`](./docs/PERFORMANCE.md).

## Feature completeness

zig_iroh implements **259 of 381** required rows of iroh's user-facing feature surface (**68%**) — a working subset that names the gaps it does not yet cover. Full per-subsystem detail is in [`docs/COMPLETION_MATRIX.md`](./docs/COMPLETION_MATRIX.md).
